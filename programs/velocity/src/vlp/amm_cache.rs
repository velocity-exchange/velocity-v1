...
impl CacheInfo {
    pub fn get_oracle_source(&self) -> VelocityResult<OracleSource> {
        OracleSource::try_from(self.oracle_source)
    }

    pub fn oracle_id(&self) -> VelocityResult<OracleIdentifier> {
        let oracle_source = self.get_oracle_source()?;
        Ok((self.oracle, oracle_source))
    }

    pub fn get_last_available_amm_token_amount(&self) -> VelocityResult<i128> {
    ...
    pub fn update_perp_market_fields(&mut self, perp_market: &PerpMarket) -> VelocityResult<()> {
...
impl<'a> AccountZeroCopy<'a, CacheInfo, AmmCacheFixed> {
    pub fn get_for_market_index(&self, market_index: u16) -> VelocityResult<&CacheInfo> {
    ...
    pub fn check_settle_staleness(
        &self,
        slot: u64,
        threshold_slot_diff: u64,
    ) -> VelocityResult<()> {
    ...
    pub fn check_perp_market_staleness(&self, slot: u64, threshold: u64) -> VelocityResult<()> {
        for (i, cache_info) in self.iter().enumerate() {
            if cache_info.slot == 0 {
                continue;
            }
    ...
    pub fn check_oracle_staleness(&self, slot: u64, threshold: u64) -> VelocityResult<()> {
        for (i, cache_info) in self.iter().enumerate() {
            if cache_info.slot == 0 {
                continue;
            }
...
impl<'a> AccountZeroCopyMut<'a, CacheInfo, AmmCacheFixed> {
    pub fn get_for_market_index_mut(
        &mut self,
        market_index: u16,
    ) -> VelocityResult<&mut CacheInfo> {
        let pos = {
