...
| oracle-F4 #72 | Pyth Lazer oracle handler now reads the signed `Confidence` payload property (previously ignored) and persists `conf = max(signed_confidence, bid/ask spread, 20bps floor)` — never smaller than the prior fabricated value. No layout/IDL change |
