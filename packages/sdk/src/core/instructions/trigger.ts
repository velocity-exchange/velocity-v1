 ...
 *   checked for the authority-wide equity breaker. A risk-increasing trigger order is
 *   cancelled rather than activated while the breaker is tripped, and a cancel that
 *   observes the owner below its raw equity floor arms the breaker inline.
...
export async function buildTriggerOrderInstruction(args: {
	...
	/** the user's relay trigger conditions PDA. Omit it to pass the program id
	 * as a placeholder. */
	triggerConditions?: PublicKey;
	/** the market's crank-conditions PDA, which holds the reservoir. Omit it to
	 * pass the program id as a placeholder. */
	crankConditions?: PublicKey;
}): Promise<TransactionInstruction> {
	// An omitted optional relay account, which is the user's trigger conditions
	// or the market's crank conditions, encodes as the program id. That is
	// anchor's `None`.
