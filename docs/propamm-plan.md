...
- [ ] Router fill ix: quote CPIs → split (S6 waterfall) → clamp → execute CPIs → apply balance changes; passes the loaded-user set into quote/execute CPIs and rejects balance changes for unloaded users; `probe_quoter` dies when this lands (S1)
