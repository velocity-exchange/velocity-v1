...
mod get_margin_ratio {
    ...
    fn test() {
        ...
        let margin_ratio_initial = perp_market
            .get_margin_ratio(BASE_PRECISION, MarginRequirementType::Initial)
            .unwrap();

        let margin_ratio_maintenance = perp_market
            .get_margin_ratio(BASE_PRECISION, MarginRequirementType::Maintenance)
            .unwrap();

        let margin_ratio_fill = perp_market
            .get_margin_ratio(BASE_PRECISION, MarginRequirementType::Fill)
            .unwrap();
...
mod get_trigger_price {
    ...
    fn test_get_last_funding_basis() {
        ...
        assert_eq!(last_funding_basis, 1089046); // basis adjusted for offset denominator 3333 => 10.95% APR
        ...
        assert_eq!(last_funding_basis, 544523);
...
mod mark_twap_sample_cap {
    ...
    /// of the 15% oracle band) after a >27h gap. Uncapped, the gap hands that one
    ...
    fn cap_bounds_a_single_post_gap_crank_sample() {
        let now = 100_000_i64; // long gap since t=0
