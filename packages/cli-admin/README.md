...
velocity-admin clob-market update-config <market> [--tick-size <n>] [--step-size <n>] [--min-order-size <n>] [--blocking-min-size <n>] [--default-activation-delay <slots>] [--max-activation-delay <slots>] [--unknown-user-grace-slots <n>] [--evict-threshold <n>] [--max-quote-levels <n>] [--max-execute-fills <n>] [--max-execute-users <n>] [--reservation-grace-slots <n>]  # warm/cold admin; retune a live book through velocity's update_perp_market_clob_book_config, which CPIs the CLOB's update_market_v0 as the book's config authority (the market's quoter slab) and rewrites velocity's copy of the book's rules in the same instruction; only the flags passed are written. --reservation-grace-slots is how long a taker remainder's claim on the depth it crosses is honoured, so it is what bounds a cross crank that never lands (0..150)
velocity-admin clob-market resize <market> <newCapacity>  # warm/cold admin; grow a live book's order arena through velocity's resize_perp_market_clob_book; the admin pays the rent; one call grows it by at most about 116 orders
