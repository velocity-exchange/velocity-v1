...
describe('post only maker order w/ amm fulfillments', () => {
	...
	beforeEach(async () => {
		...
		await setFeedPriceNoProgram(contextWrapper, 32.821, solUsd);
	});
	...
	it('long', async () => {
		...
		setFeedPriceNoProgram(contextWrapper, newOraclePrice, solUsd);
		...
		await setFeedPriceNoProgram(contextWrapper, newOraclePrice, solUsd);
