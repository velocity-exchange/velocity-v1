...
Set **`CI gate`** as the _only_ required status check; remove the individual job checks
...
### 5. Two dead tests to delete (removed features) — **done**

Both removed (deleted, not rewritten — the underlying features are gone from this fork):

- realtime-archiver `swift-message-parser.test.ts` — deleted. Its 2 `describe.skip` blocks decoded
  a **removed** wire format (`OrderMetadataAndMessage` is `order_message_str: String` now, not a
  borsh enum) through an Anchor-0.29 IDL path that crashes under Anchor 1.0.
- drift-rs `dlob::tests::dlob_l2_snapshot_max_leverage_filtering` — deleted. Tested a max-leverage
  L2 filtering feature that **doesn't exist in this fork**.
...
- **drift-rs `event_subscriber` 4 base64-log `#[ignore]`s** (`parses_*` are fixed/un-ignored;
  these 4 are `rpc_tests`-gated and need _real captured mainnet tx logs_ that can't be
