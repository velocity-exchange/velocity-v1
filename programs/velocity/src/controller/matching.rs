//! Fill engine.
//!
//! Three explicit, hardcoded fill paths — there is no general
//! continuous-curve clearing algorithm (no bisection, no price-domain
//! search):
//!
...
//! - [`router_take`]: the router fill — internal [`RouterQuoter`] books
//!   (vAMM ladder, DLOB-order bridges) plus externally-quoted CPI books
//!   split by priority tier; internal allocations settle in place, external
//!   ones return for the CPI execute leg.
//!
...
fn discrete_level(
    ...
    side: PositionDirection,
    price_cap: u64,
) -> VelocityResult<Option<DiscreteLevel>> {
...
/// at-or-better bar).
///
/// Like [`match_take`], this touches only quoter-side state; taker position
