...
export class AnchorProvider {
	constructor() {
		throw new Error(
			'AnchorProvider is not supported in the browser build. Use `DriftCore` (pure builders/decoders/PDAs) instead of `VelocityClient`.'
		);
	}
}
...
export class Program<_T = any> {
	constructor() {
		throw new Error(
			'Program is not supported in the browser build. Use `DriftCore` (pure builders/decoders/PDAs) instead of `VelocityClient`.'
		);
	}
}
