...
| #276 funding-pause-settlement-sites | Fix OtterSec finding #65: the shared `settle_funding_payment` helper — which folds a position's already-accrued cumulative funding into its quote balance — ran unconditionally on the public fill, perp-position-transfer, PnL-settle, and liquidation paths while the dedicated `settle_funding_payment` crank was gated by `funding_not_paused`. The helper now takes a `funding_paused` flag (threaded from `State::funding_paused`) and no-ops under the exchange-wide funding pause, so pending funding is left unapplied: `last_cumulative_funding_rate` is untouched and the next settlement after the pause lifts covers the full elapsed interval (mirrors the spot-interest freeze in `funding-pause-enforcement`). A position's total economic value (settled + unsettled funding) is unchanged, so SDK funding/margin math is unaffected — but a client simulating the exact on-chain `quote_asset_amount` mutation during a pause must note funding is no longer folded in until the pause lifts. Intended consequence: a liquidation during a pause values a position without realizing its pending funding. The dedicated `settle_funding_payment` crank stays gated. No layout/IDL/error-enum/SDK-surface change |
