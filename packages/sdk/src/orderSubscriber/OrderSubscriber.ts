...
export class OrderSubscriber {
	...
	public async getDLOB(slot: number): Promise<DLOB> {
		...
		// Classification during insertion depends on auction progress, so seed
		// the clock before the first order. Assigning it afterward cannot undo a
		// taking order that was promoted into a resting node list.
		try {
			dlob.slotDurationState = this.velocityClient.getStateAccount();
		} catch {
			// A custom source may build before State subscribes. The baseline
			// fallback stays compatible, and DLOBSubscriber attaches State once
			// it is available.
		}
