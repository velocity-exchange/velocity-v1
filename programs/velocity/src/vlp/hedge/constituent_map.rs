...
impl<'a> ConstituentMap<'a> {
    #[track_caller]
    #[inline(always)]
    pub fn get_ref(&self, constituent_index: &u16) -> VelocityResult<Ref<'_, Constituent>> {
    ...
    #[track_caller]
    #[inline(always)]
    pub fn get_ref_mut(&self, market_index: &u16) -> VelocityResult<RefMut<'_, Constituent>> {
    ...
    ) -> VelocityResult<ConstituentMap<'a>> {
        let mut constituent_map: ConstituentMap = ConstituentMap(BTreeMap::new());

        let constituent_discriminator: &[u8] = Constituent::DISCRIMINATOR;
