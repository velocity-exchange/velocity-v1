use crate::{
    math::{
        constants::{
            AMM_RESERVE_PRECISION, PEG_PRECISION, PRICE_PRECISION, PRICE_PRECISION_I64,
            PRICE_PRECISION_U64, QUOTE_PRECISION,
        },
        oracle::OracleValidity,
    },
    state::{
        oracle::{HistoricalOracleData, OraclePriceData},
        perp_market::{MarketStats, PerpMarket},
        user::PerpPosition,
    },
    vlp::amm::math::amm::*,
};

#[test]
fn calculate_amm_available_guards() {
    let prev = 1656682258;
    let _now = prev + 2;

    // let px = 32 * PRICE_PRECISION;

    let mut market: PerpMarket = PerpMarket::default_btc_test();

    let _oracle_price_data = OraclePriceData {
        price: (34 * PRICE_PRECISION) as i64,
        confidence: PRICE_PRECISION_U64 / 100,
        delay: 1,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };

    assert_eq!(market.amm.net_revenue_since_last_funding, 0);
    assert_eq!(market.amm.total_fee_minus_distributions, 0);

    assert_eq!(!market.has_too_much_drawdown().unwrap(), true);

    market.amm.net_revenue_since_last_funding = -100_000_000_000;
    market.amm.total_fee_minus_distributions = 100_000_000_000;

    assert_eq!(!market.has_too_much_drawdown().unwrap(), false);

    market.amm.net_revenue_since_last_funding = -10_000_000_000;
    market.amm.total_fee_minus_distributions = 100_000_000_000;

    assert_eq!(!market.has_too_much_drawdown().unwrap(), false);

    market.amm.net_revenue_since_last_funding = -5_000_000_000;
    market.amm.total_fee_minus_distributions = 100_000_000_000;

    assert_eq!(!market.has_too_much_drawdown().unwrap(), false);

    market.amm.net_revenue_since_last_funding = -1_000_000_000;
    market.amm.total_fee_minus_distributions = 100_000_000_000;

    assert_eq!(!market.has_too_much_drawdown().unwrap(), true);

    market.amm.net_revenue_since_last_funding = -1_000_000_000;
    market.amm.total_fee_minus_distributions = 1_000_000;

    assert_eq!(!market.has_too_much_drawdown().unwrap(), true);

    market.amm.net_revenue_since_last_funding = -6_000_000_000;
    market.amm.total_fee_minus_distributions = 1_000_000;

    assert_eq!(!market.has_too_much_drawdown().unwrap(), false);

    market.amm.net_revenue_since_last_funding = -5_000;
    market.amm.total_fee_minus_distributions = -9279797219;

    assert_eq!(!market.has_too_much_drawdown().unwrap(), true); // too small net_revenue_since_last_funding drawdown

    market.amm.net_revenue_since_last_funding = -88_000_000_000;
    market.amm.total_fee_minus_distributions = -9279797219;

    assert_eq!(!market.has_too_much_drawdown().unwrap(), false); // too small net_revenue_since_last_funding drawdown
}

#[test]
fn calculate_net_user_pnl_test() {
    let prev = 1656682258;
    let _now = prev + 3600;

    let px = 32 * PRICE_PRECISION;

    let amm = AMM {
        base_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        quote_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        peg_multiplier: PEG_PRECISION,
        ..AMM::default_test()
    };
    let market_stats = MarketStats {
        historical_oracle_data: HistoricalOracleData {
            last_oracle_price_twap: px as i64,
            last_oracle_price_twap_ts: prev,

            ..HistoricalOracleData::default()
        },
        mark_std: PRICE_PRECISION as u64,
        last_mark_price_twap_ts: prev,
        funding_period: 3600_i64,
        ..MarketStats::default()
    };
    let market = PerpMarket {
        amm,
        market_stats,
        ..PerpMarket::default()
    };

    let oracle_price_data = OraclePriceData {
        price: (34 * PRICE_PRECISION) as i64,
        confidence: PRICE_PRECISION_U64 / 100,
        delay: 1,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };

    let net_user_pnl = calculate_net_user_pnl(
        &market.amm,
        oracle_price_data.price,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
    )
    .unwrap();
    assert_eq!(net_user_pnl, 0);

    let market: PerpMarket = PerpMarket::default_btc_test();
    let net_user_pnl = calculate_net_user_pnl(
        &market.amm,
        market.market_stats.historical_oracle_data.last_oracle_price,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
    )
    .unwrap();
    assert_eq!(net_user_pnl, -400000000); // down $400

    let net_user_pnl = calculate_net_user_pnl(
        &market.amm,
        17501 * PRICE_PRECISION_I64,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
    )
    .unwrap();
    assert_eq!(net_user_pnl, 1499000000); // up $1499
}

#[test]
fn calculate_expiry_price_long_imbalance_with_loss_test() {
    let prev = 1656682258;
    let _now = prev + 3600;

    // imbalanced short, no longs
    // btc
    let oracle_price_data = OraclePriceData {
        price: (22050 * PRICE_PRECISION) as i64,
        confidence: 0,
        delay: 2,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };

    let market_position = PerpPosition {
        market_index: 0,
        base_asset_amount: (12295081967 / 2_i64),
        quote_asset_amount: -193688524588, // $31506 entry price
        ..PerpPosition::default()
    };

    let market = PerpMarket {
        market_index: 0,
        amm: AMM {
            base_asset_reserve: 512295081967,
            quote_asset_reserve: 488 * AMM_RESERVE_PRECISION,
            sqrt_k: 500 * AMM_RESERVE_PRECISION,
            peg_multiplier: 22_100_000_000,
            base_asset_amount_with_amm: (12295081967_i128),
            max_spread: 1000,
            // assume someone else has other half same entry,
            ..AMM::default()
        },
        margin_ratio_initial: 1000,
        margin_ratio_maintenance: 500,
        imf_factor: 1000, // 1_000/1_000_000 = .001
        unrealized_pnl_initial_asset_weight: 100,
        unrealized_pnl_maintenance_asset_weight: 100,
        quote_asset_amount: market_position.quote_asset_amount as i128 * 2,
        order_step_size: 1,
        ..PerpMarket::default()
    };

    let mut expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        0,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    let reserve_price = market.amm.reserve_price().unwrap();
    let (terminal_price, _, _) = calculate_terminal_price_and_reserves(&market.amm).unwrap();
    let oracle_price = oracle_price_data.price;

    assert_eq!(expiry_price, 22049999999);
    assert_eq!(terminal_price, 20076684570);
    assert_eq!(oracle_price, 22050000000);
    assert_eq!(reserve_price, 21051929600);

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        111_111_110, // $111
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    assert_eq!(expiry_price, 22049999999); // same price

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        1_111_111_110, // $1,111
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    assert_eq!(expiry_price, 22049999999); // same price again

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        (111_111_110 * QUOTE_PRECISION) as i128,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    assert_eq!(expiry_price, 22049999999);
    assert_eq!(expiry_price, oracle_price - 1); // more longs than shorts, bias = -1
}

#[test]
fn calculate_expiry_price_long_imbalance_test() {
    let prev = 1656682258;
    let _now = prev + 3600;

    // imbalanced short, no longs
    // btc
    let oracle_price_data = OraclePriceData {
        price: (22050 * PRICE_PRECISION) as i64,
        confidence: 0,
        delay: 2,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };

    let market_position = PerpPosition {
        market_index: 0,
        base_asset_amount: (12295081967 / 2_i64),
        quote_asset_amount: -103688524588, // $16,866.66 entry price
        ..PerpPosition::default()
    };

    let market = PerpMarket {
        market_index: 0,
        amm: AMM {
            base_asset_reserve: 512295081967,
            quote_asset_reserve: 488 * AMM_RESERVE_PRECISION,
            sqrt_k: 500 * AMM_RESERVE_PRECISION,
            peg_multiplier: 22_100_000_000,
            base_asset_amount_with_amm: (12295081967_i128),
            max_spread: 1000,
            // assume someone else has other half same entry,
            ..AMM::default()
        },
        margin_ratio_initial: 1000,
        margin_ratio_maintenance: 500,
        imf_factor: 1000, // 1_000/1_000_000 = .001
        unrealized_pnl_initial_asset_weight: 100,
        unrealized_pnl_maintenance_asset_weight: 100,
        quote_asset_amount: market_position.quote_asset_amount as i128 * 2,
        order_step_size: 1,
        ..PerpMarket::default()
    };

    let mut expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        0,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    let reserve_price = market.amm.reserve_price().unwrap();
    let (terminal_price, _, _) = calculate_terminal_price_and_reserves(&market.amm).unwrap();
    let oracle_price = oracle_price_data.price;

    assert_eq!(expiry_price, 16866666665);
    assert_eq!(terminal_price, 20076684570);
    assert_eq!(oracle_price, 22050000000);
    assert_eq!(reserve_price, 21051929600);

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        111_111_110, // $111
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    assert_eq!(expiry_price, 16875703702); // better price

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        1_111_111_110, // $1,111
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    assert_eq!(expiry_price, 16957037035); // even better price

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        (111_111_110 * QUOTE_PRECISION) as i128,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    assert_eq!(expiry_price, 22049999999);
    assert_eq!(expiry_price, oracle_price - 1); // more longs than shorts, bias = -1
}

#[test]
fn calculate_expiry_price_test() {
    let prev = 1656682258;
    let _now = prev + 3600;

    let px = 32 * PRICE_PRECISION;

    let amm = AMM {
        base_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        quote_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        peg_multiplier: PEG_PRECISION,
        ..AMM::default_test()
    };
    let _market_stats = MarketStats {
        historical_oracle_data: HistoricalOracleData {
            last_oracle_price_twap: px as i64,
            last_oracle_price_twap_ts: prev,

            ..HistoricalOracleData::default()
        },
        mark_std: PRICE_PRECISION as u64,
        last_mark_price_twap_ts: prev,
        ..MarketStats::default()
    };
    let order_step_size: u64 = 1;

    let oracle_price_data = OraclePriceData {
        price: (34 * PRICE_PRECISION) as i64,
        confidence: PRICE_PRECISION_U64 / 100,
        delay: 1,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };

    let mut expiry_price =
        calculate_expiry_price(&amm, oracle_price_data.price, 0, 0, 0, order_step_size).unwrap();

    assert_eq!(expiry_price, oracle_price_data.price);

    expiry_price = calculate_expiry_price(
        &amm,
        oracle_price_data.price,
        111111110,
        0,
        0,
        order_step_size,
    )
    .unwrap();

    assert_eq!(expiry_price, oracle_price_data.price);

    // imbalanced short, no longs
    // btc
    let oracle_price_data = OraclePriceData {
        price: (22050 * PRICE_PRECISION) as i64,
        confidence: 0,
        delay: 2,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };

    let market_position = PerpPosition {
        market_index: 0,
        base_asset_amount: -(122950819670000 / 2_i64),
        quote_asset_amount: 153688524588, // $25,000 entry price
        ..PerpPosition::default()
    };

    let market = PerpMarket {
        market_index: 0,
        amm: AMM {
            base_asset_reserve: 512295081967,
            quote_asset_reserve: 488 * AMM_RESERVE_PRECISION,
            sqrt_k: 500 * AMM_RESERVE_PRECISION,
            peg_multiplier: 22_100_000_000,
            base_asset_amount_with_amm: -(12295081967_i128),
            max_spread: 1000,
            // assume someone else has other half same entry,
            ..AMM::default()
        },
        margin_ratio_initial: 1000,
        margin_ratio_maintenance: 500,
        imf_factor: 1000, // 1_000/1_000_000 = .001
        unrealized_pnl_initial_asset_weight: 100,
        unrealized_pnl_maintenance_asset_weight: 100,
        quote_asset_amount: market_position.quote_asset_amount as i128 * 2,
        order_step_size: 1,
        ..PerpMarket::default()
    };

    let mut expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        0,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    let reserve_price = market.amm.reserve_price().unwrap();
    let (terminal_price, _, _) = calculate_terminal_price_and_reserves(&market.amm).unwrap();
    let oracle_price = oracle_price_data.price;

    assert_eq!(expiry_price, 25000000001);
    assert_eq!(terminal_price, 22100000000);
    assert_eq!(oracle_price, 22050000000);
    assert_eq!(reserve_price, 21051929600);

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        111_111_110, // $111
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    // 250000000000814 - 249909629631346 = 90370369468 (~$9 improved)
    assert_eq!(expiry_price, 24990962964); // better price

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        1_111_111_110, // $1,111
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    // 250000000000814 - 249096296297998 = 903703702816 (~$90 improved)
    assert_eq!(expiry_price, 24909629630); // even better price

    expiry_price = calculate_expiry_price(
        &market.amm,
        oracle_price_data.price,
        (111111110 * QUOTE_PRECISION) as i128,
        market.quote_asset_amount,
        market.net_unsettled_funding_pnl,
        market.order_step_size,
    )
    .unwrap();

    assert_eq!(expiry_price, 22050000001);
    assert_eq!(expiry_price, oracle_price + 1); // more shorts than longs, bias = +1
}

#[test]

fn calc_delayed_mark_twap_tests() {
    let prev = 1656682258;
    let now = prev + 60;
    let amm = AMM {
        base_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        quote_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        peg_multiplier: PRICE_PRECISION,
        base_spread: 655,  //base spread is .065%,
        max_spread: 65535, //base spread is 6.5%
        ..AMM::default()
    };
    let market_stats = MarketStats {
        mark_std: PRICE_PRECISION as u64,
        last_bid_price_twap: 21999 * PRICE_PRECISION as u64,
        last_ask_price_twap: 22001 * PRICE_PRECISION as u64,
        last_mark_price_twap: 22000 * PRICE_PRECISION as u64,
        last_mark_price_twap_ts: prev - 3600,
        historical_oracle_data: HistoricalOracleData {
            last_oracle_price: 22850 * PRICE_PRECISION as i64,
            last_oracle_price_twap: 22900 * PRICE_PRECISION as i64,
            last_oracle_price_twap_ts: prev,
            ..HistoricalOracleData::default()
        },
        funding_period: 3600,
        ..MarketStats::default()
    };
    let mut market = PerpMarket {
        amm,
        market_stats,
        order_tick_size: 1,
        ..PerpMarket::default()
    };
    let px = 22850 * PRICE_PRECISION as i64;
    market.amm.peg_multiplier = px as u128;
    let trade_direction = PositionDirection::Long;
    market
        .market_stats
        .update_mark_twap_from_estimates(
            &market.amm,
            now,
            Some(px as u64),
            Some(trade_direction),
            None,
            market.order_tick_size,
        )
        .unwrap();

    assert_eq!(market.market_stats.last_bid_price_twap, 22850013657);
    assert_eq!(market.market_stats.last_mark_price_twap, 22850013657);
    assert_eq!(market.market_stats.last_ask_price_twap, 22850013657);

    assert_eq!(
        market.market_stats.last_mark_price_twap as i64
            - market
                .market_stats
                .historical_oracle_data
                .last_oracle_price_twap,
        -49986343
    );
}

#[test]
fn calc_mark_std_tests() {
    let prev = 1656682258;
    let mut now = prev + 60;
    let amm = AMM {
        base_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        quote_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        peg_multiplier: PRICE_PRECISION,
        base_spread: 65535, //max base spread is 6.5%
        ..AMM::default()
    };
    let market_stats = MarketStats {
        mark_std: PRICE_PRECISION as u64,
        historical_oracle_data: HistoricalOracleData {
            last_oracle_price: PRICE_PRECISION as i64,
            last_oracle_price_twap: PRICE_PRECISION as i64,
            last_oracle_price_twap_5min: PRICE_PRECISION as i64,
            ..HistoricalOracleData::default()
        },
        last_mark_price_twap_ts: prev,
        funding_period: 0,
        ..MarketStats::default()
    };
    // funding_period is intentionally 0 — matches the old AMM-default in this
    // test, which makes `calculate_new_oracle_price_twap` short-circuit
    // (`from_start = 0`) so the oracle twap tracks the live oracle price.
    let mut market = PerpMarket {
        amm,
        market_stats,
        order_tick_size: 1,
        ..PerpMarket::default()
    };

    market
        .market_stats
        .update_mark_std(now, PRICE_PRECISION_U64 * 23, 0, 0)
        .unwrap();
    assert_eq!(market.market_stats.mark_std, 23000000);

    market.market_stats.mark_std = PRICE_PRECISION_U64;
    market.market_stats.last_mark_price_twap_ts = now - 60;
    market
        .market_stats
        .update_mark_std(now, PRICE_PRECISION_U64 * 2, 0, 0)
        .unwrap();
    assert_eq!(market.market_stats.mark_std, 2000000);

    let mut px = PRICE_PRECISION_U64;
    let stop_time = now + 3600 * 2;
    while now <= stop_time {
        now += 1;
        if now % 15 == 0 {
            px = px * 1012 / 1000;
            market.market_stats.historical_oracle_data.last_oracle_price =
                market.market_stats.historical_oracle_data.last_oracle_price * 10119 / 10000;
        } else {
            px = px * 100000 / 100133;
            market.market_stats.historical_oracle_data.last_oracle_price =
                market.market_stats.historical_oracle_data.last_oracle_price * 100001 / 100133;
        }

        let oracle_price_data = OraclePriceData {
            price: market.market_stats.historical_oracle_data.last_oracle_price,
            confidence: 0,
            delay: 2,
            has_sufficient_number_of_data_points: true,
            sequence_id: None,
        };
        let mm_oracle_price_data = MMOraclePriceData::new(
            oracle_price_data.price,
            3,
            1,
            OracleValidity::default(),
            oracle_price_data,
        )
        .unwrap();
        market
            .market_stats
            .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
            .unwrap();

        market.amm.peg_multiplier = px as u128;
        let trade_direction = PositionDirection::Long;
        market
            .market_stats
            .update_mark_twap_from_estimates(
                &market.amm,
                now,
                Some(px),
                Some(trade_direction),
                None,
                market.order_tick_size,
            )
            .unwrap();
    }
    assert_eq!(now, 1656689519);
    assert_eq!(px, 39397);
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap,
        40194
    );
    assert_eq!(market.market_stats.last_mark_price_twap, 39449);

    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap_5min,
        46930
    );
    assert_eq!(market.market_stats.last_mark_price_twap_5min, 46051);

    assert_eq!(market.market_stats.last_bid_price_twap, 39449);
    assert_eq!(market.market_stats.last_ask_price_twap, 39449);

    assert_eq!(market.market_stats.mark_std, 6676);

    // sol price looking thinkg
    let mut px: u64 = 31_936_658;
    let stop_time = now + 3600 * 2;
    assert_eq!(market.amm.reserve_price().unwrap(), 39397);
    market.amm.peg_multiplier = 31_986_658;
    assert_eq!(market.amm.reserve_price().unwrap(), 31986658);
    market.market_stats.historical_oracle_data.last_oracle_price = 31986658;

    while now <= stop_time {
        now += 1;
        if now % 15 == 0 {
            px = 31_986_658; //31.98
            market.market_stats.historical_oracle_data.last_oracle_price = (px - 1000000) as i64;
            market.amm.peg_multiplier = px as u128;
            let amm_reserve_price = market.amm.reserve_price().unwrap();
            let (amm_bid_price, amm_ask_price) = market
                .amm
                .bid_ask_price(amm_reserve_price, 0, 0, 0)
                .unwrap();
            msg!("bid={:?} ask={:?}", amm_bid_price, amm_ask_price);

            assert!(amm_bid_price <= px);
            assert!(amm_ask_price >= px);

            let trade_direction = PositionDirection::Long;
            market
                .market_stats
                .update_mark_twap_from_estimates(
                    &market.amm,
                    now,
                    Some(px),
                    Some(trade_direction),
                    None,
                    market.order_tick_size,
                )
                .unwrap();
        }
        if now % 189 == 0 {
            px = 31_883_651; //31.88
            market.amm.peg_multiplier = px as u128;
            market.market_stats.historical_oracle_data.last_oracle_price = (px + 1000000) as i64;

            let amm_reserve_price = market.amm.reserve_price().unwrap();
            let (amm_bid_price, amm_ask_price) = market
                .amm
                .bid_ask_price(amm_reserve_price, 0, 0, 0)
                .unwrap();
            msg!("bid={:?} ask={:?}", amm_bid_price, amm_ask_price);
            assert!(amm_bid_price <= px);
            assert!(amm_ask_price >= px);

            let trade_direction = PositionDirection::Short;
            market
                .market_stats
                .update_mark_twap_from_estimates(
                    &market.amm,
                    now,
                    Some(px),
                    Some(trade_direction),
                    None,
                    market.order_tick_size,
                )
                .unwrap();
        }
    }
    assert_eq!(now, 1656696720);
    assert_eq!(px, 31986658);
    assert_eq!(market.market_stats.mark_std, 13244);

    // sol price looking thinkg
    let mut px: u64 = 31_936_658;
    let stop_time = now + 3600 * 2;
    while now <= stop_time {
        now += 1;
        if now % 2 == 1 {
            px = 31_986_658; //31.98
            market.amm.peg_multiplier = px as u128;

            market.market_stats.historical_oracle_data.last_oracle_price = (px - 1000000) as i64;
            let trade_direction = PositionDirection::Long;
            market
                .market_stats
                .update_mark_twap_from_estimates(
                    &market.amm,
                    now,
                    Some(px),
                    Some(trade_direction),
                    None,
                    market.order_tick_size,
                )
                .unwrap();
        }
        if now % 2 == 0 {
            px = 31_883_651; //31.88
            market.amm.peg_multiplier = px as u128;

            market.market_stats.historical_oracle_data.last_oracle_price = (px + 1000000) as i64;
            let trade_direction = PositionDirection::Short;
            market
                .market_stats
                .update_mark_twap_from_estimates(
                    &market.amm,
                    now,
                    Some(px),
                    Some(trade_direction),
                    None,
                    market.order_tick_size,
                )
                .unwrap();

            let mark_twap = market.market_stats.last_mark_price_twap;

            market
                .market_stats
                .update_oracle_std(now, px + 1000000, mark_twap, mark_twap)
                .unwrap();
        }
    }
    assert_eq!(now, 1656703921);
    assert_eq!(px, 31986658);
    assert_eq!(market.market_stats.mark_std, 68672); //.068
    assert_eq!(market.market_stats.oracle_std, 965665); // used mark twap ema tho
}

#[test]
fn update_mark_twap_tests() {
    let prev = 0;

    let mut now = 1;

    let mut oracle_price_data = OraclePriceData {
        price: 40_021_280 * PRICE_PRECISION_I64 / 1_000_000,
        confidence: PRICE_PRECISION_U64 / 100,
        delay: 1,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };
    let mm_oracle_price_data = MMOraclePriceData::new(
        oracle_price_data.price,
        3,
        1,
        OracleValidity::default(),
        oracle_price_data,
    )
    .unwrap();

    // $40 everything init
    let amm = AMM {
        quote_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        base_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        peg_multiplier: 40 * PEG_PRECISION,
        base_spread: 800,
        max_spread: 10000,
        ..AMM::default()
    };
    let market_stats = MarketStats {
        last_mark_price_twap: 40 * PRICE_PRECISION_U64,
        last_bid_price_twap: (40 * PRICE_PRECISION_U64) - 1,
        last_ask_price_twap: 40 * PRICE_PRECISION_U64,
        last_mark_price_twap_ts: prev,
        historical_oracle_data: HistoricalOracleData {
            last_oracle_price: (40 * PRICE_PRECISION) as i64,
            last_oracle_price_twap: (40 * PRICE_PRECISION) as i64,
            last_oracle_price_twap_ts: prev,
            ..HistoricalOracleData::default()
        },
        funding_period: 3600,
        ..MarketStats::default()
    };
    let mut market = PerpMarket {
        amm,
        market_stats,
        order_tick_size: 1,
        ..PerpMarket::default()
    };
    market
        .market_stats
        .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
        .unwrap();
    assert_eq!(
        market.market_stats.historical_oracle_data.last_oracle_price,
        oracle_price_data.price
    );
    assert_eq!(
        market.market_stats.historical_oracle_data.last_oracle_price,
        40_021_280 * PRICE_PRECISION_I64 / 1_000_000
    );

    let trade_price = 40_051_280 * PRICE_PRECISION_U64 / 1_000_000;
    let trade_direction = PositionDirection::Long;

    let old_mark_twap = market.market_stats.last_mark_price_twap;
    let new_mark_twap = market
        .market_stats
        .update_mark_twap_from_estimates(
            &market.amm,
            now,
            Some(trade_price),
            Some(trade_direction),
            None,
            market.order_tick_size,
        )
        .unwrap();
    let new_bid_twap = market.market_stats.last_bid_price_twap;
    let new_ask_twap = market.market_stats.last_ask_price_twap;

    assert_eq!(old_mark_twap, 40_000_000);
    assert_eq!(new_mark_twap, 40_000_010);
    assert!(new_mark_twap > old_mark_twap);
    assert_eq!(new_ask_twap, 40_000_015);
    assert_eq!(new_bid_twap, 40_000_005);
    assert!(new_bid_twap < new_ask_twap);

    while now < 3600 {
        now += 1;
        market
            .market_stats
            .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
            .unwrap();
        market
            .market_stats
            .update_mark_twap_from_estimates(
                &market.amm,
                now,
                Some(trade_price),
                Some(trade_direction),
                None,
                market.order_tick_size,
            )
            .unwrap();
    }

    let new_oracle_twap = market
        .market_stats
        .historical_oracle_data
        .last_oracle_price_twap;
    let new_mark_twap = market.market_stats.last_mark_price_twap;
    let new_bid_twap = market.market_stats.last_bid_price_twap;
    let new_ask_twap = market.market_stats.last_ask_price_twap;

    assert!(new_bid_twap <= new_ask_twap);
    assert_eq!((new_bid_twap + new_ask_twap) / 2, new_mark_twap);
    assert!((new_oracle_twap as u64) < new_mark_twap); // funding in favor of maker?
    assert_eq!(new_oracle_twap, 40008161);
    assert_eq!(new_bid_twap, 40014547);
    assert_eq!(new_mark_twap, 40024054); // ~ 2 cents above oracle twap
    assert_eq!(new_ask_twap, 40033561);
    assert_eq!(market.market_stats.mark_std, 27230);
    assert_eq!(market.market_stats.oracle_std, 3120);

    let trade_price_2 = 39_971_280 * PRICE_PRECISION_U64 / 1_000_000;
    let trade_direction_2 = PositionDirection::Short;
    oracle_price_data = OraclePriceData {
        price: 39_991_280 * PRICE_PRECISION_I64 / 1_000_000,
        confidence: PRICE_PRECISION_U64 / 80,
        delay: 14,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };
    let mm_oracle_price_data = MMOraclePriceData::new(
        oracle_price_data.price,
        15,
        1,
        OracleValidity::default(),
        oracle_price_data,
    )
    .unwrap();

    while now <= 3600 * 2 {
        now += 1;
        market
            .market_stats
            .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
            .unwrap();
        if now % 200 == 0 {
            market
                .market_stats
                .update_mark_twap_from_estimates(
                    &market.amm,
                    now,
                    Some(trade_price_2),
                    Some(trade_direction_2),
                    None,
                    market.order_tick_size,
                )
                .unwrap();
            // ~2 cents below oracle
        }
    }

    let new_oracle_twap = market
        .market_stats
        .historical_oracle_data
        .last_oracle_price_twap;
    let new_mark_twap = market.market_stats.last_mark_price_twap;
    let new_bid_twap = market.market_stats.last_bid_price_twap;
    let new_ask_twap = market.market_stats.last_ask_price_twap;

    assert_eq!(new_bid_twap, 39_989_386);
    assert_eq!(new_ask_twap, 40_000_774);
    assert!(new_bid_twap < new_ask_twap);
    assert_eq!((new_bid_twap + new_ask_twap) / 2, new_mark_twap);

    assert_eq!(new_oracle_twap, 39_998_518);
    assert_eq!(new_mark_twap, 39995080);

    assert!((new_oracle_twap as u64) >= new_mark_twap); // funding in favor of maker
    assert_eq!(market.market_stats.oracle_std, 7240);
    assert_eq!(market.market_stats.mark_std, 24457);
}

#[test]
fn calc_oracle_twap_tests() {
    let prev = 1656682258;
    let now = prev + 3600;

    let px = 32 * PRICE_PRECISION;

    let amm = AMM {
        base_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        quote_asset_reserve: 2 * AMM_RESERVE_PRECISION,
        peg_multiplier: PEG_PRECISION,
        ..AMM::default()
    };
    let market_stats = MarketStats {
        historical_oracle_data: HistoricalOracleData {
            last_oracle_price_twap: px as i64,
            last_oracle_price_twap_ts: prev,
            ..HistoricalOracleData::default()
        },
        mark_std: PRICE_PRECISION as u64,
        last_mark_price_twap_ts: prev,
        funding_period: 3600_i64,
        ..MarketStats::default()
    };
    let mut market = PerpMarket {
        amm,
        market_stats,
        ..PerpMarket::default()
    };
    let mut oracle_price_data = OraclePriceData {
        price: (34 * PRICE_PRECISION) as i64,
        confidence: PRICE_PRECISION_U64 / 100,
        delay: 1,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };
    let mm_oracle_price_data = MMOraclePriceData::new(
        oracle_price_data.price,
        2,
        1,
        OracleValidity::default(),
        oracle_price_data,
    )
    .unwrap();
    let _new_oracle_twap = market
        .market_stats
        .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
        .unwrap();
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap,
        (34 * PRICE_PRECISION - PRICE_PRECISION / 100) as i64
    );

    // let after_ts = amm.historical_oracle_data.last_oracle_price_twap_ts;
    market.market_stats.last_mark_price_twap_ts = now - 60;
    market
        .market_stats
        .historical_oracle_data
        .last_oracle_price_twap_ts = now - 60;
    // let after_ts_2 = amm.historical_oracle_data.last_oracle_price_twap_ts;
    oracle_price_data = OraclePriceData {
        price: (31 * PRICE_PRECISION) as i64,
        confidence: 0,
        delay: 2,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };
    let mm_oracle_price_data = MMOraclePriceData::new(
        oracle_price_data.price,
        3,
        1,
        OracleValidity::default(),
        oracle_price_data,
    )
    .unwrap();

    // let old_oracle_twap_2 = amm.historical_oracle_data.last_oracle_price_twap;
    let _new_oracle_twap_2 = market
        .market_stats
        .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
        .unwrap();
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap,
        33940167
    );
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap_5min,
        33392001
    );
    assert_eq!(market.market_stats.oracle_std, 2_990_000);
    let _new_oracle_twap_2 = market
        .market_stats
        .update_oracle_twap(&market.amm, now + 60 * 5, &mm_oracle_price_data, None, None)
        .unwrap();

    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap,
        33695154
    );
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap_5min,
        31 * PRICE_PRECISION_I64
    );
    assert_eq!(market.market_stats.oracle_std, 2940167);

    oracle_price_data = OraclePriceData {
        price: (32 * PRICE_PRECISION) as i64,
        confidence: 0,
        delay: 2,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };
    let mm_oracle_price_data = MMOraclePriceData::new(
        oracle_price_data.price,
        3,
        1,
        OracleValidity::default(),
        oracle_price_data,
    )
    .unwrap();
    let _new_oracle_twap_2 = market
        .market_stats
        .update_oracle_twap(
            &market.amm,
            now + 60 * 5 + 60,
            &mm_oracle_price_data,
            None,
            None,
        )
        .unwrap();
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap_5min,
        31200001
    );
    assert_eq!(market.market_stats.oracle_std, 1695154); // ~$1.7 of std
}

#[test]
fn calc_oracle_twap_clamp_update_tests() {
    let prev = 1667387000;
    let mut now = prev + 1;

    // let oracle_price_data = OraclePriceData {
    //     price: 13_021_280 * PRICE_PRECISION_I64 / 1_000_000,
    //     confidence: PRICE_PRECISION_U64 / 100,
    //     delay: 1,
    //     has_sufficient_number_of_data_points: true,
    // };

    // $13 everything init
    let amm = AMM {
        quote_asset_reserve: 200 * AMM_RESERVE_PRECISION,
        base_asset_reserve: 200 * AMM_RESERVE_PRECISION,
        peg_multiplier: 13 * PEG_PRECISION,
        base_spread: 0,
        ..AMM::default()
    };
    let market_stats = MarketStats {
        last_mark_price_twap: 13 * PRICE_PRECISION_U64,
        last_bid_price_twap: 13 * PRICE_PRECISION_U64,
        last_ask_price_twap: 13 * PRICE_PRECISION_U64,
        last_mark_price_twap_ts: prev,
        historical_oracle_data: HistoricalOracleData {
            last_oracle_price: (13 * PRICE_PRECISION) as i64,
            last_oracle_price_twap: (13 * PRICE_PRECISION) as i64,
            last_oracle_price_twap_5min: (13 * PRICE_PRECISION) as i64,
            last_oracle_price_twap_ts: prev,
            ..HistoricalOracleData::default()
        },
        funding_period: 3600,
        ..MarketStats::default()
    };
    let mut market = PerpMarket {
        amm,
        market_stats,
        ..PerpMarket::default()
    };

    // price jumps 10x
    let oracle_price_data = OraclePriceData {
        price: 130 * PRICE_PRECISION_I64 + 873,
        confidence: PRICE_PRECISION_U64 / 10,
        delay: 1,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };
    let mm_oracle_price_data = MMOraclePriceData::new(
        oracle_price_data.price,
        2,
        1,
        OracleValidity::default(),
        oracle_price_data,
    )
    .unwrap();

    while now < prev + 3600 {
        market
            .market_stats
            .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
            .unwrap();
        now += 1;
    }
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap,
        18_143_130
    );
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap_5min,
        23_536_961
    );
    assert_eq!(market.market_stats.last_oracle_normalised_price, 24_188_600);

    while now < prev + 3600 * 2 {
        market
            .market_stats
            .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
            .unwrap();
        now += 1;
    }

    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap,
        25_322_529
    );
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap_5min,
        32_850_803
    );
    assert_eq!(market.market_stats.last_oracle_normalised_price, 33_760_245);

    while now < prev + 3600 * 10 {
        market
            .market_stats
            .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
            .unwrap();
        now += 1;
    }

    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap,
        129_282_724
    );
    assert_eq!(
        market
            .market_stats
            .historical_oracle_data
            .last_oracle_price_twap_5min,
        129_900_874
    );
    assert_eq!(
        market.market_stats.last_oracle_normalised_price,
        129_900_873
    );
}

#[test]
fn test_last_oracle_conf_update() {
    let prev = 1667387000;
    let now = prev + 1;

    let amm = AMM {
        quote_asset_reserve: 200 * AMM_RESERVE_PRECISION,
        base_asset_reserve: 200 * AMM_RESERVE_PRECISION,
        peg_multiplier: 13 * PEG_PRECISION,
        base_spread: 0,
        ..AMM::default()
    };
    let market_stats = MarketStats {
        last_mark_price_twap: 13 * PRICE_PRECISION_U64,
        last_bid_price_twap: 13 * PRICE_PRECISION_U64,
        last_ask_price_twap: 13 * PRICE_PRECISION_U64,
        last_mark_price_twap_ts: prev,
        historical_oracle_data: HistoricalOracleData {
            last_oracle_price: (13 * PRICE_PRECISION) as i64,
            last_oracle_price_twap: (13 * PRICE_PRECISION) as i64,
            last_oracle_price_twap_5min: (13 * PRICE_PRECISION) as i64,
            last_oracle_price_twap_ts: prev,
            ..HistoricalOracleData::default()
        },
        funding_period: 3600,
        ..MarketStats::default()
    };
    let mut market = PerpMarket {
        amm,
        market_stats,
        ..PerpMarket::default()
    };

    // price jumps 10x
    let oracle_price_data = OraclePriceData {
        price: 130 * PRICE_PRECISION_I64 + 873,
        confidence: PRICE_PRECISION_U64 / 10,
        delay: 1,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };
    let mm_oracle_price_data = MMOraclePriceData::new(
        oracle_price_data.price,
        2,
        1,
        OracleValidity::default(),
        oracle_price_data,
    )
    .unwrap();
    market
        .market_stats
        .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
        .unwrap();

    assert_eq!(market.market_stats.last_oracle_conf_pct, 7692);

    // price jumps 10x
    let oracle_price_data = OraclePriceData {
        price: 130 * PRICE_PRECISION_I64 + 873,
        confidence: 1,
        delay: 5,
        has_sufficient_number_of_data_points: true,
        sequence_id: None,
    };

    let mm_oracle_price_data = MMOraclePriceData::new(
        oracle_price_data.price,
        2,
        1,
        OracleValidity::default(),
        oracle_price_data,
    )
    .unwrap();

    // unchanged if now hasnt changed
    market
        .market_stats
        .update_oracle_twap(&market.amm, now, &mm_oracle_price_data, None, None)
        .unwrap();
    assert_eq!(market.market_stats.last_oracle_conf_pct, 7692);
    market
        .market_stats
        .update_oracle_twap(&market.amm, now + 1, &mm_oracle_price_data, None, None)
        .unwrap();

    assert_eq!(market.market_stats.last_oracle_conf_pct, 7692 - 7692 / 20); // 7287

    // longer time between update means delay is faster
    market
        .market_stats
        .update_oracle_twap(&market.amm, now + 60, &mm_oracle_price_data, None, None)
        .unwrap();

    assert_eq!(
        market.market_stats.last_oracle_conf_pct,
        7307 - 7307 / 5 + 1
    ); //5847
}
