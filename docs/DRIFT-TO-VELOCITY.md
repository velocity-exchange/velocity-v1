...
- **`SpotMarket.if_last_settle_vault_amount: u64`** replaces the 8-byte trailing padding.
  ...
  settle can no longer inflate the cap while legitimate stakes still lift it. The same
  `min(live_if_vault, this)` base is used by `calculate_if_shares_lost` (the unstake-cancel
  forfeiture), so a donation sandwiched around a staker's cancel can no longer inflate the shares
  that staker forfeits. No new instruction — maintained automatically by the IF
