...
| `SignedMsgUserOrders` | 76 + 40 per entry | A legacy account is 76 + 24 per entry until a resize. See §5.3. |
...
- `SignedMsgUserOrders` layout (feat/propamm). `SignedMsgOrderId` grows from 24 to 40 bytes. It
  ...
  The header's `padding: u32` is now `version: u32`, at the same offset, and
  `initialize_signed_msg_user_orders` writes 1. An account is in the legacy layout only when
  `version` is 0 and its size is exactly `8 + 32 + 4 + 32 + 24 * len`. Every other account is
  ...
  header `len`. The SDK's `decodeSignedMsgUserOrdersAccount` reads both layouts, and the
  signed-message subscribers use it in place of the Anchor coder.
