use {
    crate::{
        ...
        declare_vault_seeds,
        error::ErrorCode,
        math::token_a_to_token_b,
        state::{
            events::{ManagerBorrowRecord, ManagerUpdateBorrowRecord},
            FeeUpdateProvider, FeeUpdateStatus, VaultProtocolProvider,
        },
        token_cpi::TokenTransferCPI,
        validate,
        velocity_cpi::ManagerBorrowCPI,
...
pub fn manager_borrow<'info>(
    ...
    validate!(
        velocity_spot_market_vault.mint == borrow_spot_market.mint,
    ...
    ctx.velocity_withdraw(borrow_spot_market_index, borrow_amount)?;
