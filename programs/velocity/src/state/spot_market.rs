...
pub struct SpotMarket {
    ...
    /// Insurance-fund vault balance recorded at the end of the last revenue
    /// settle. `settle_revenue_to_insurance_fund` is the only writer, and it
    /// writes the live vault balance plus the amount that settle transfers in.
    /// No other instruction touches it, so it lags the live vault by up to one
    /// `revenue_settle_period`.
    ///
    /// The per-period revenue-settle APR cap is sized off
    /// `min(live_if_vault, this)`. Both endpoints of the period must hold the
    /// balance, so the cap only counts capital that stayed in the fund across
    /// the whole period. A donation spiked into the live vault right before a
    /// settle is absent from the older snapshot and cannot lift the cap. A
    /// donation that does survive a full period already belongs to the stakers
    /// pro rata, so the fund really is that large. The `min` with the live
    /// balance also makes the cap self-correct after a loss draw, which is why
    /// the draw paths need no bookkeeping here.
    ///
    ...
    /// because the settle that reads `0` still writes the snapshot.
    ///
...
impl SpotMarket {
    ...
    pub fn get_scaled_initial_asset_weight(&self, oracle_price: i64) -> VelocityResult<u32> {
    ...
    pub fn get_deposits(&self) -> VelocityResult<u128> {
    ...
    pub fn get_available_deposits(&self) -> VelocityResult<u128> {
    ...
    pub fn get_utilization(self) -> VelocityResult<u128> {
        let deposit_token_amount =
            get_token_amount(self.deposit_balance, &self, &SpotBalanceType::Deposit)?;
    ...
    pub fn is_healthy_utilization(self) -> VelocityResult<bool> {
    ...
    pub fn get_min_borrow_rate(self) -> VelocityResult<u32> {
