...
- Spot trading charges no fee. The swap fee is hardcoded to zero
  (`let fee = 0_u64;`, `instructions/user/swap.rs`), and there is no spot
  ...
  former `spot_fee_pool` slot is now `padding_former_spot_fee_pool`.
...
- The protocol's automatic fee share is one half of taker fees
  (`SHARE_OF_FEES_ALLOCATED_TO_DRIFT = 1/2`, a constant the redesign removed). That
...
| Claim | Symbol | Line |
|---|---|---|
| Swap fee hardcoded zero | `let fee = 0_u64;` | `instructions/user/swap.rs` |