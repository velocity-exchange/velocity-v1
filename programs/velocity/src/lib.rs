...
declare_id!("vELoC1audYbSYVRXn1vPaV8Axoa9oU6BYmNGZZBDZ1P");
...
pub mod velocity {
    ...
    pub fn delete_signed_msg_user_orders<'c: 'info, 'info>(
        ctx: Context<'info, DeleteSignedMsgUserOrders>,
    ) -> Result<()> {
    ...
    pub fn force_cancel_orders<'c: 'info, 'info>(
        ctx: Context<'info, ForceCancelOrder<'info>>,
    ) -> Result<()> {
...
security_txt! {
    ...
    contacts: "email:security@lumencollective.xyz,link:https://docs.velocity.exchange/protocol/risk-and-safety/bug-bounty",
