...
mod fill_gates_apply_to_a_reducing_fill {
    use {
        crate::{
            controller::{orders::fulfill_perp_order, position::PositionDirection},
            create_anchor_account_info,
    ...
    /// Fills one reducing taker order against one reducing maker.
    ///
    /// `borrow_market_stale_by` sets how long ago market 1 — the market the taker
    /// borrows in — last accrued interest. `collateral_oracle_slots_behind` sets how
    /// far the oracle of market 2 — the market holding the taker's collateral — lags
    /// the fill slot. `OracleGuardRails::default()` calls a margin price stale past
    /// 120 slots.
    fn reducing_fill(
        borrow_market_stale_by: i64,
        collateral_oracle_slots_behind: u64,
    ) -> Result<(), ErrorCode> {
        let mut perp_oracle_price = get_pyth_price(100, 6);
        perp_oracle_price.posted_slot = SLOT;
        let perp_oracle_key =
            Pubkey::from_str("J83w4HKfqxwcq3BEMMkPFSppX3gqekLyLJBexebFVkix").unwrap();
        create_anchor_account_info!(
            perp_oracle_price,
        ...
        let mut collateral_oracle_price = get_pyth_price(100, 6);
        collateral_oracle_price.posted_slot = SLOT - collateral_oracle_slots_behind;
        let collateral_oracle_key =
            Pubkey::from_str("BAtFj4kQttZRVep3UZS2aZRDixkGYgWsbqTBVDbnSsPF").unwrap();
        create_anchor_account_info!(
        ...
        let oracle_infos = [perp_oracle_info, collateral_oracle_info];
        let mut oracle_map =
            OracleMap::load(&mut oracle_infos.iter().peekable(), SLOT, None).unwrap();
        ...
        market.amm.max_base_asset_reserve = 200 * AMM_RESERVE_PRECISION;
        market.amm.min_base_asset_reserve = 0;
        ...
        // 50% utilization under a 70%/6%/100% curve, so an un-booked interval is
        // measurable rather than rounding to nothing.
        let mut borrow_market = SpotMarket {
            ...
            last_interest_ts: (NOW - borrow_market_stale_by) as u64,
            historical_oracle_data: HistoricalOracleData::default_price(100 * PRICE_PRECISION_I64),
            ..SpotMarket::default_base_market()
        };
        create_anchor_account_info!(borrow_market, SpotMarket, borrow_market_info);

        let mut collateral_market = SpotMarket {
            market_index: 2,
            oracle: collateral_oracle_key,
            oracle_source: OracleSource::PythLazer,
            last_interest_ts: NOW as u64,
            historical_oracle_data: HistoricalOracleData::default_price(100 * PRICE_PRECISION_I64),
            ..SpotMarket::default_base_market()
        };
        create_anchor_account_info!(collateral_market, SpotMarket, collateral_market_info);

        let spot_market_map = SpotMarketMap::load_multiple(
            vec![
                &quote_market_info,
                &borrow_market_info,
                &collateral_market_info,
            ],
            true,
        )
        .unwrap();
        ...
        // Long 2 base, selling 1: the fill halves the position and leaves a margin
        // requirement the collateral still has to cover.
        let mut taker = User {
            orders: get_orders!(Order {
                market_index: 0,
                status: OrderStatus::Open,
        ...
        // Short 1 base, buying 1: the maker seat closes as well.
        let maker_key = Pubkey::default();
        let maker_authority =
            Pubkey::from_str("J83w4HKfqxwcq3BEMMkPFSppX3gqekLyLJBexebFVkix").unwrap();
        let mut maker = User {
        ...
        create_anchor_account_info!(maker, User, maker_info);
        let makers_and_referrers = UserMap::load_one(&maker_info).unwrap();

        let mut maker_stats = UserStats {
            authority: maker_authority,
            ..UserStats::default()
        };
        create_anchor_account_info!(maker_stats, UserStats, maker_stats_info);
        let maker_and_referrer_stats = UserStatsMap::load_one(&maker_stats_info).unwrap();

        let mut taker_stats = UserStats::default();
        let mut filler = User::default();
        let mut filler_stats = UserStats::default();
        let (taker_key, _, filler_key) = super::get_user_keys();
    ...
    /// The window market 1 earns from its own rate ceiling.
    fn borrow_market_window() -> i64 {
        max_spot_interest_staleness_for_margin(&SpotMarket {
            max_borrow_rate: 1_000_000,
            ..SpotMarket::default_base_market()
        })
        .unwrap()
    ...
    #[test]
    fn every_input_current_fills() {
        assert_eq!(reducing_fill(0, 0), Ok(()));
    }

    /// OtterSec #148: un-booked interest understates the borrow, and this handler
    /// makes no spot market refreshable.
    #[test]
    fn an_un_cranked_borrow_market_stops_it() {
        assert_eq!(
            reducing_fill(borrow_market_window() + 1, 0),
            Err(ErrorCode::SpotMarketInterestStaleForMargin),
            "a reducing fill must not value a borrow through an un-cranked index"
        );
    }

    /// OtterSec #143: a deposit whose oracle is stale for margin contributes zero
    /// collateral, so the taker fails on what its valid collateral covers rather
    /// than on the stale price. Its only collateral is in market 2 here, so the
    /// fill stops for want of collateral.
    #[test]
    fn a_stale_collateral_oracle_is_not_credited() {
        assert_eq!(
            reducing_fill(0, 130),
            Err(ErrorCode::InsufficientCollateral),
            "a stale deposit oracle must not price the taker through its margin \
             requirement on a reducing fill"
        );
    }
}
...
/// The CLOB arm of the same rule `maker_floor_prune` pins for the DLOB.
///
/// A book cannot be pruned order by order — its execute sweeps best-first and
/// velocity cannot tell it to pass over one maker — so the router instead
/// stops routing at the first order the post-fill checks would refuse. These
/// pin where that cut lands.
pub mod clob_floor_depth_clamp {
    use {
        super::*,
        crate::{
            controller::{orders::clob_unverifiable_floor_depth, position::PositionDirection},
            create_anchor_account_info,
            error::VelocityResult,
            math::constants::{
                AMM_RESERVE_PRECISION, BASE_PRECISION_I64, BASE_PRECISION_U64, PEG_PRECISION,
            ...
            state::{
                ...
                perp_market::{MarketStats, AMM},
                perp_market_map::PerpMarketMap,
                prop_amm::{ClobRestingOrderV0, ClobUserRefV0},
                pyth_lazer_oracle::PythLazerOracle,
                spot_market::{SpotBalanceType, SpotMarket},
                spot_market_map::SpotMarketMap,
    ...
    const MAKER_A: &str = "J83w4HKfqxwcq3BEMMkPFSppX3gqekLyLJBexebFVkix";
    const MAKER_B: &str = "My11111111111111111111111111111111111111111";

    /// `resting` is the book's sweep, best price first, as
    /// `(maker index, base, price)`. `makers` gives each maker its resting
    /// position in the market and its equity floor; a floor of 0 leaves the
    /// maker verifiable, and any other value is unverifiable here because the
    /// oracle is read far past the slot it was posted at.
    ///
    ...
    fn cap_and_clear(
        ...
        let mut oracle_price = get_pyth_price(100, 6);
        let oracle_price_key = Pubkey::from_str(MAKER_A).unwrap();
        create_anchor_account_info!(
            oracle_price,
            &oracle_price_key,
            PythLazerOracle,
        ...
        let build = |authority: Pubkey, position_base: i64, floor: u64| User {
            authority,
            equity_floor: floor,
            perp_positions: get_positions(PerpPosition {
                market_index: 0,
        ...
        let authority_a = Pubkey::from_str(MAKER_A).unwrap();
        let authority_b = Pubkey::from_str(MAKER_B).unwrap();
        let mut maker_a = build(authority_a, makers[0].0, makers[0].1);
        let mut maker_b = build(authority_b, makers[1].0, makers[1].1);
        // Distinct account keys: the two-argument form of the macro keys
        // every account at `Pubkey::default()`, which would collapse the two
        // makers into one entry in the map.
        let maker_a_key = Pubkey::new_unique();
        let maker_b_key = Pubkey::new_unique();
        create_anchor_account_info!(maker_a, &maker_a_key, User, maker_a_info);
        create_anchor_account_info!(maker_b, &maker_b_key, User, maker_b_info);
        let mut makers_and_referrers = UserMap::load_one(&maker_a_info).unwrap();
        let second = UserMap::load_one(&maker_b_info).unwrap();
        makers_and_referrers.0.extend(second.0);

        let authorities = [authority_a, authority_b];
        let user_ref_index = makers_and_referrers.user_ref_index().unwrap();
        let resolve = |user: &ClobUserRefV0| -> VelocityResult<Pubkey> {
            Ok(*user_ref_index
    ...
    const PRICE: u64 = 100 * PRICE_PRECISION_U64;

    #[test]
    fn an_unfloored_book_is_never_clamped() {
        // Nobody carries a floor, so nothing is held back and the router may
        ...
        assert_eq!(cap, None, "an unfloored book takes its full depth");
    }

    #[test]
    fn a_risk_increasing_order_at_the_top_closes_the_book() {
        // The floored maker is already short and its sell adds to that, so
        // the post-fill check refuses the first order in the sweep. There is
        // no depth in front of it, and truncating to anything above zero
        // would route straight into the order that reverts the fill.
        let cap = cap(
            &[(0, BASE_PRECISION_U64, PRICE)],
    ...
    #[test]
    fn the_cut_keeps_the_depth_in_front_of_it() {
        // A clear maker rests ahead of the floored one. That depth is
        // reachable without touching the order that reverts, so it stays
        // routable and only the tail is given up.
        let cap = cap(
            &[
                (1, BASE_PRECISION_U64, PRICE),
                (0, BASE_PRECISION_U64, PRICE),
            ],
            [(-BASE_PRECISION_I64, FLOOR), (BASE_PRECISION_I64, 0)],
        );
        assert_eq!(cap, Some(BASE_PRECISION_U64));
    }

    #[test]
    fn reducing_orders_are_judged_against_what_the_ones_ahead_leave_behind() {
        // The floored maker is long 1 with two resting sells of 0.75. The
        // first reduces and stays routable; the second is judged against the
        // +0.25 the first leaves behind, where 0.75 flips it short, so the
        // book is cut there. Judging each order against the original position
        // would admit both and hand the taker a fill that reverts.
        let cap = cap(
            &[
                (0, 3 * BASE_PRECISION_U64 / 4, PRICE),
                (0, 3 * BASE_PRECISION_U64 / 4, PRICE),
            ],
            [(BASE_PRECISION_I64, FLOOR), (0, 0)],
        );
    ...
    #[test]
    fn a_floored_maker_that_only_reduces_is_not_clamped_at_all() {
        // Long 1 with two sells of 0.25: the pair still leaves the maker
        // long, so the post-fill check admits both and clamping either would
        // needlessly stop a floored maker from reducing.
        let cap = cap(
            &[
                (0, BASE_PRECISION_U64 / 4, PRICE),
                (0, BASE_PRECISION_U64 / 4, PRICE),
            ],
            [(BASE_PRECISION_I64, FLOOR), (0, 0)],
        );
