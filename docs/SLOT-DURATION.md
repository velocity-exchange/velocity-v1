...
Solana's slot time drops from 400ms to 350, 300, 250, and 200ms through the IBRL feature gates.
This document covers where the current slot length lives, how wall-clock rules keep their meaning
at every gate, and the types that stop new code from assuming a slot length again.

Solana's slot counter is the only clock a program can read cheaply. Every time-based rule in this
codebase was therefore written as a slot count, under the assumption that one slot takes 400ms.
"Stale after about 5 seconds" became `10`, "a one-minute liquidation ramp" became `150`, and "idle
after an hour" became `9000`. The number and the meaning were fused, and the fusion held only
while the assumption did. As each gate activates, a raw slot count changes meaning with no
announcement on chain: oracle staleness windows tighten until fills and liquidations revert,
user-protection ramps halve, auctions finish in half the intended time, and per-slot rate limits
...
The whole slot transition is therefore a data change. Each gate activation needs one
permissionless sync instruction, no constant retunes, and no redeploy. Code that compares a
duration against a slot count without converting does not compile.
...
(offsets pinned by `const_assert_eq!` and the native-offset guard test):
...
- `slot_duration_transition_slots` (`[u64; 4]`, bytes 1520..1552) holds the first slot of each
  ...
  worth at slot N" (`SlotClock::slot_duration_at`) and "how much wall-clock time passed between
  two slots" (`SlotClock::elapsed`, integrated per regime).
- The legacy staging trio remains as a fallback for accounts written by the first implementation
  and for older readers: `slot_duration_ms` (`u16`, bytes 1506..1508), `pending_slot_duration_ms`
  (`u16`, bytes 1508..1510), and `slot_duration_effective_slot` (`u64`, bytes 1512..1520). A
  `slot_duration_ms` of `0` means unset and resolves to the 400ms baseline, so there was no
  migration. The sync instruction keeps these fields coherent, but once any archive entry exists
  the archive wins.

Never read the fields directly. `State::slot_clock()` returns the full clock, and
`State::slot_duration()` resolves the live duration from it against the Clock sysvar, so **State
switches itself at each boundary with no boundary-timed transaction**. The resolution lives in one
place, `math::time::SlotClock`, shared by `slot_duration()`, the native fast-path reader, the
foreign-account reader vaults uses, and the off-chain mirrors.

The writer is `sync_state_slot_duration`. It is permissionless, and the admin CLI wraps it as
`velocity-admin exchange sync-slot-duration <ms>`. Anyone can crank it once per gate, and the
...
1. **Fixed gate set.** The accounts struct constrains the feature account to the feature-gate
   ...
   (`ibrl_slot_duration_ms`), so there is nothing to mistype. Recorded transition slots must be
   monotonic. A forward sync can skip an abandoned gate once the last recorded boundary is live,
   and a missing older gate can be backfilled later. Neither path can make the active duration
   slower. Re-syncing an already-recorded gate is an idempotent no-op that revalidates the
   recorded slot against the feature account.
2. **Effective slot derived from the gate and the `EpochSchedule` sysvar.** The handler verifies
   ...
   does **not** require the following epoch to have begun: recording the boundary in advance is the
   point. Because everything accepted is fixed by the feature account and the sysvar, admin
   authorization would add a failure mode (a missed multisig) without adding security, which is why
   the instruction is permissionless.

**Lockstep matters because no direction of mismatch is safe** between the live `slot_duration_ms`
and the real slot length. A conversion serves both risk *ceilings*, where a shorter oracle
staleness window is safer, and user-protection *minima*, where a longer liquidation ramp or grace
period is safer. The two want opposite errors. The maximum allowed real oracle age is
`(window_ms / live_ms) x real_slot_ms`, which gives two cases.

- **The live value runs ahead of the chain** (200ms while the chain is still 400ms). A 48s oracle
  window becomes `48000/200 x 400 = 96s`. It *widens* and accepts staler oracles, which is an
  unsafe ceiling. Ramps *lengthen* at the same time, which is safe for the user.
- **The live value runs behind the chain** (400ms while the chain is already 200ms). The oracle
  window becomes `48000/400 x 200 = 24s`. It *tightens*, which is a safe ceiling at a liveness
  cost. Ramps and grace periods *shorten* to half their intended wall clock, which is an unsafe
  minimum.

Synchronizing during the warmup makes the effective value match the chain at the exact boundary,
so neither lag is ever live in steady state. That is why the effective slot is derived from the
gate rather than from a hand-timed transaction at the boundary.

Runbook, in full:

```
# once the Feature Gate Tracker shows the gate activated. Any signer works, keepers included.
velocity-admin exchange sync-slot-duration 350   # then 300, 250, 200 as each is activated
```

Each transition needs one sync transaction, ideally during the activation epoch. State switches
automatically at the effective slot. A late sync still records the exact historical boundary,
because the feature account keeps its activation slot forever, so elapsed-time math is repaired
retroactively. The only cost of lateness is that the live duration lagged in the interim. No
guard-rail retune, per-market update, or bot restart is needed, because the off-chain mirrors read
the same fields from their state subscription.

For a cluster whose legacy staging fields already reached a faster regime before this archive is
initialized, synchronize the currently active fastest gate first, then backfill the older gates.
For example, a cluster already at 200ms must sync 200ms before 350, 300, and 250ms. The handler
checks the proposed clock at the current slot and rejects any archive write that would move it
backward, so a permissionless caller cannot restore a slower duration even temporarily. The legacy
staging trio is then rebuilt from the archive rather than allowed to block the repair.
...
Solana's slot *counter* does not record that earlier slots were longer. Converting a whole
measured delta at one endpoint duration therefore mis-times any interval that crosses a
transition. The result is an under-count. That is safe for elapsed and ramp sites, because it
favors the user. It is unsafe for oracle-freshness gates, because they briefly accept a slightly
staler oracle.
...
behavior, identity at the 400ms baseline.
...
**`SlotClock`** is the cluster clock: the four transition slots plus the legacy staging fields.
It is built only from `State`, through `State::slot_clock()` or `SlotClock::from_state_fields`. It
answers `slot_duration_at(slot)` for any slot and integrates `elapsed(start, end)` and
`elapsed_slot_delta(delta, end)` piecewise across the recorded regimes. Every measured-interval
site takes a `SlotClock`. `SlotClock::baseline()` is the all-zero 400ms clock for tests and for
contexts with no `State`.

**`SlotDuration`** is one slot length. Program logic obtains it from the clock, through
...
`StoredSlotDuration<u8, 400>` still occupies one byte, and a raw `10` there means 4,000ms.
...
existing `u8`, `i64`, and `u64` decoded shapes. Signed fields with sentinel values are
deliberately not modeled as ordinary durations and continue through `DelayOverride`.
...
`Millis`: same-slot idempotence checks, blockhash validity windows, and "a fill lands ~1 slot
ahead" estimates. Two kinds of numbers exist and the type system keeps them apart rather than
flattening them. `Order.auction_duration` stores 400ms *units* rather than slots. See
[Auctions](#auctions-the-u8-stores-400ms-units).
...
The author's decision is "give relayers about 4 seconds of grace". The constant states that
decision directly, and the measured age floats with the chain. Ten elapsed slots read as 4.0s at
400ms and 2.0s at 200ms, and an interval that crosses a gate sums each side at its own duration.

At the baseline every conversion is the identity on the historical values, because 10 slots is
4,000ms, exactly the old constant. The program's unit suite passes unmodified, which is the proof
that the types change nothing until the first gate flips.
...
400ms era does not need to know the conversion functions exist. The type error names the method,
and the method's signature demands the `SlotDuration` that only `State` supplies.
...
The worst-case rounding error at any gate is under one slot. A 4s window at 350ms floors to 11
slots, which is 3.85s, where the exact figure is 11.43 slots.

## Legacy stored fields and the 400ms encoding

Code constants were rewritten in milliseconds, because nothing stores them. Admin-set onchain
fields could not be. They hold live values on mainnet, and some are too narrow to hold
milliseconds at all: `liquidation_duration` is a `u8`, and 60 seconds is 60,000. Those fields keep
their compact encoding in units of `STORED_UNIT_MS` = 400ms, the historical slot length. Each
field's typed getter is the only place that encoding appears.
...
The `400` is a storage codec detail, the same way `PRICE_PRECISION` is. An admin types seconds in
the CLI, which encodes them on the way in, logic compares `Millis`, and the chain stores a compact
integer. A new compact stored duration encodes its chosen quantum in
`StoredSlotDuration<T, SLOT_MS>`. Use a native millisecond integer instead when arbitrary
millisecond precision matters more than width. To retire a legacy field's encoding, carve a
ms-native replacement field from padding, give the getter a fallback, and re-set the value once.
The typed getter is the only reader, so nothing else in the codebase moves.
...
(`get_auction_duration` counts calibration steps, and admin minimums like
`min_perp_auction_duration` are already stored in the same units). It is interpolated at fill time
as wall-clock progress:
`SlotClock::elapsed(order.slot, now)` over `auction_duration * 400ms`, exact across transitions.
User-supplied `OrderParams.auction_duration` carries the same 400ms-unit meaning.

The u8 therefore keeps the full historical range at every gate. The derived maximum is 180 units,
which is 72s, and the representable ceiling is 255 units, which is 102s. No wall-clock compression
exists at any slot duration. An earlier design stored actual slots and clamped to about 51s at
200ms. `BID_ASK_TWAP_MIN_QUOTE_REST` (9.6s) is measured piecewise from the same clock as the
auction ramp and the `SafeTriggerOrder` horizon, so a quote must rest at least as long as the
auction it can move at every gate (OtterSec #146).

One related field does not scale. `Order.posted_slot_tail` is a mod-256 slot stamp, which is a
field width rather than a constant, so the order-age window it can express shrinks in wall clock
as slots get faster: 102s at 400ms and 51s at 200ms. The rest requirement, 48 actual slots at
...
- bot liveness tripwires counted in slot ticks. A dead-feed restart that fires sooner at faster
  slots is acceptable, and each site documents that.

One legacy encoding is kept for compatibility. `block_operation`'s funding gate compares elapsed
400ms periods against `funding_period`, which is denominated in seconds. The previous code
compared a raw slot count against the seconds value, a unit mismatch that widened as slots got
faster. The gate now holds its current wall-clock width, about 40% of the funding period, at every
slot duration.
...
baseline paths are a liveness note rather than a safety gap. At 200ms they want an oracle cranked
within about 2s. keep-rs's liquidator and filler re-sample the slot duration on a live cadence.
The liquidator's rate limiter reads a shared value the main loop refreshes, and the filler
refreshes on an elapsed-slot config tick. A process that spans a gate activation therefore picks
up the new duration without a restart.
...
Getting it wrong does not compile. A raw threshold does not compare against anything the rest of
the system produces, because measured deltas arrive as `Millis` and thresholds are consumed as
...
- **SDK** ([`math/time.ts`](../packages/sdk/src/math/time.ts)): branded `Millis` (a `BN`) and
  ...
  transition archive first (legacy staging fields as fallback), and `elapsedMillis` /
  ...
  optional trailing `SlotDurationState`: the decoded `State`, or its slot-duration fields. It
  defaults to the baseline, so state-less callers stay correct until the first gate.
  `SLOT_TIME_ESTIMATE_MS` is deprecated. No constant can replace it, only the live fields. `currentSlotDuration` / `currentSlotClock` resolve the live duration for any client
...
- **Bots** (`apps/dlob-server`, `apps/keeper-bots-v2`): pacing and threshold constants are
  ...
  defaults are the exception: they encode to the onchain 400ms units (`msToSlotsCeilNum(ms,
  SLOT_DURATION_BASELINE)`), deliberately independent of the live duration. `keeper-bots-v2`
  resolves live durations through the SDK's `currentSlotDuration(client, currentSlot)` helper
  (pass a live chain slot so a switch is applied, and an unavailable state or slot falls back to
  the 400ms baseline). `dlob-server`'s remaining live-duration read, the vAMM quote-spread
  estimate, still calls `activeSlotDurationFromState` directly and so has no dead-feed fallback. Operator config
  knobs are ms (`fillAttemptIntervalMs`, `deriskAuctionDurationMs`).
- **Rust bots** (`rust/keep-rs`, `rust/swift`, `rust/velocity-rs`): use the program's `SlotClock`
  directly through `velocity_rs::slot_clock_from_state` or `VelocityClient::slot_clock`. keep-rs's
  oracle-validity gates and liquidation-fee mirror, the AMM quoting projection, and the swift
  local margin simulation all measure through it. Forward window conversions keep the live
...
bump without a flag day. No constant can replace it.
...
Code that already holds a decoded `State` and a slot calls the underlying
`activeSlotDurationFromState(state, slot)` instead: `velocityClient`, `user.ts`,
`math/auction.ts`, `adminClient`, `cli-admin`, and `dlob-server`. That is the same staged-flip
resolution without the subscription lookup or the fallback, so those sites must handle an absent
`State` themselves. Use the resolver whenever the state comes from a subscription that might not
be ready. The resolver enforces two rules so callers cannot get them wrong.
...
| --- | --- | --- |
| ms -> slots (`msToSlotsNum`, `msToSlotsCeilNum`) | **fewer slots**, so the window closes sooner | staleness gates, rate limits, auction durations |
| slots -> ms (`millisFromSlots`, a bare multiply) | **up to 2x more ms**, so the window stays open longer | countdowns, cache TTLs, signing budgets |

"Fewer slots" is the safe side for a threshold that should be conservative, and the wrong side
for a countdown shown to a user. "More ms" is always the wrong side for anything a user relies on
to still be valid. At a real 200ms, a client that reports 400ms per slot promises twice the wall
clock the user has, and the user keeps signing an already-expired transaction.
...
| Thing | Value |
| --- | --- |
| Clock | `State.slot_duration_transition_slots` (`[u64; 4]`, bytes 1520..1552, `0` = unsynced), with the legacy staging trio as fallback |
| Writer | `sync_state_slot_duration`, permissionless. Gate keys are fixed on the accounts struct, and the effective slot comes from `EpochSchedule` |
| CLI | `velocity-admin exchange sync-slot-duration <ms>` |
| Duration types | `Millis` for arithmetic, `SlotClock` for measured intervals, and `StoredSlotDuration<T, SLOT_MS>` for compact account storage |
