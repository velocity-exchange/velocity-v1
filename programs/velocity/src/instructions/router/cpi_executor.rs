...
use {
    crate::{
        ...
        msg,
        state::prop_amm::{
            clob_resting_prefix, read_clob_u16, ClobUserRefV0, Direction, ExecuteArgsV0,
            ExecuteResponseV0, ExternalQuoterExecutor, QuoterSubjects, QuoterType,
            QuoterUserSetRef, QuoterV0, CLOB_MARKET_INDEX_OFFSET,
        },
        validate,
    },
    anchor_lang::prelude::*,
...
pub struct CpiQuoterExecutor<'a, 'info> {
    ...
    /// A heap slice, not the wire's fixed array: this struct lives in the
    /// fill's 4 KB stack frame.
