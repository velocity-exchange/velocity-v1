...
pub trait ZeroCopyLoader<'a, T, F> {
    fn load_zc(&'a self) -> VelocityResult<AccountZeroCopy<'a, T, F>>;
    fn load_zc_mut(&'a self) -> VelocityResult<AccountZeroCopyMut<'a, T, F>>;
}
...
pub fn load_generic<'a, 'info, F, T>(
    ...
    expected_disc: &[u8],
    program_id: Pubkey,
) -> VelocityResult<AccountZeroCopy<'a, T, F>>
where
    F: Pod + HasLen,
    T: Pod,
{
...
pub fn load_generic_mut<'a, 'info, F, T>(
    ...
    expected_disc: &[u8],
    program_id: Pubkey,
) -> VelocityResult<AccountZeroCopyMut<'a, T, F>>
where
    F: Pod + HasLen,
    T: Pod,
{
