...
Every admin-settable parameter of the Velocity program. For each one this document records which
instruction sets it, who can call that instruction, what units the value is in, what range the
program enforces on it, and what a change does to a user with an open position, a resting order,
or a pending withdrawal.

Two facts frame everything below.

1. There is no onchain timelock. The program checks the signer against the admin keys stored
...
docs) are given from the repo root. Line numbers were re-verified against the current tree when
this document was last revised. They drift as files are edited, so use them as a starting point
for `git log -L` rather than as fixed addresses.

Authority tiers. Three additive signer tiers live on `State`, and the emergency pause key sits
outside that hierarchy rather than above or below it (`auth.rs:1-14`):
...
no validation at all, onchain, after following every helper the handler calls. It is a verified
...
next X" below means that the moment any transaction executes instruction X after the parameter
write lands, the new value governs that execution. There is no epoch boundary, no cache, and no
per-user snapshot unless a row says otherwise.

## Margin and liquidation

These are the most thoroughly validated setters in the program. Perp margin ratios and liquidation
fees flow through the
...
| --- | --- | --- | --- | --- |
| `PerpMarket.margin_ratio_initial` | `update_perp_market_margin_ratio` (`admin.rs:1491`) | warm | `MARGIN_PRECISION` (1e4) | 125..=10000 (`MIN`/`MAX_MARGIN_RATIO`, `constants.rs:279-280`); strictly > maintenance; `initial * 100 > amm.max_spread` (`validation/margin.rs:22-24,30-32,44-50`) |
...
| `PerpMarket.liquidator_fee` | `update_perp_liquidation_fee` (`admin.rs:1698`) | warm | `LIQUIDATION_FEE_PRECISION` (1e6) | fee sum (all three) strictly < 100% (`admin.rs:1711-1718`); cross-checked against maintenance margin (`validation/margin.rs:34-42`) |
| `PerpMarket.if_liquidation_fee` | same handler | warm | `LIQUIDATION_FEE_PRECISION` | < 100% standalone (`admin.rs:1720-1724`); in the sum and the margin cross-check |
| `PerpMarket.protocol_liquidation_fee` | same handler | warm | `LIQUIDATION_FEE_PRECISION` | <= 10% (`admin.rs:1726-1730`); in the sum; *not* in the margin cross-check |
| `SpotMarket.liquidator_fee` | `update_spot_market_liquidation_fee` (`admin.rs:1805`) | warm | `LIQUIDATION_FEE_PRECISION` | fee sum strictly < 100% (`admin.rs:1817-1824`) and nothing else; see [missing bounds](#missing-bounds-worth-knowing-about) |
| `SpotMarket.if_liquidation_fee` | same handler | warm | `LIQUIDATION_FEE_PRECISION` | <= 10% (`admin.rs:1826-1830`); in the sum |
| `SpotMarket.protocol_liquidation_fee` | same handler | warm | `LIQUIDATION_FEE_PRECISION` | <= 10% (`admin.rs:1832-1836`); in the sum |
| `PerpMarket.imf_factor` | `update_perp_market_imf_factor` (`admin.rs:2462`) | warm | `SPOT_IMF_PRECISION` (1e6); the field doc comment says `MARGIN_PRECISION` and is wrong (see below) | <= 1_000_000 (`admin.rs:2467-2471`) |
| `PerpMarket.unrealized_pnl_imf_factor` | same handler | warm | `SPOT_IMF_PRECISION`; same wrong doc comment | <= 1_000_000 (`admin.rs:2472-2476`) |
| `PerpMarket.unrealized_pnl_initial_asset_weight` | `update_perp_market_unrealized_asset_weight` (`admin.rs:2500`) | warm | `SPOT_WEIGHT_PRECISION` (1e4) | <= 10000 and <= the maintenance weight (`admin.rs:2505-2509,2515-2519`) |
| `PerpMarket.unrealized_pnl_maintenance_asset_weight` | same handler | warm | `SPOT_WEIGHT_PRECISION` | <= 10000 and >= the initial weight (`admin.rs:2510-2519`) |
| `PerpMarket.unrealized_pnl_max_imbalance` | `update_perp_market_max_imbalances` (`admin.rs:1596`) | warm | `QUOTE_PRECISION` | <= tier insurance cap + 1 (`admin.rs:1618-1626`; tier caps `constants.rs:198-201`) |
| `PerpMarket.insurance_claim.quote_max_insurance` | same handler | warm | `QUOTE_PRECISION` | <= tier insurance cap; >= insurance already settled (`admin.rs:1618-1633`) |
| `PerpMarket.insurance_claim.max_revenue_withdraw_per_period` | same handler | warm | `QUOTE_PRECISION` | <= max(tier cap, 250 USD); >= the current period's draw (`admin.rs:1618-1626,1658`) |
| `PerpMarket.contract_tier` | `update_perp_market_contract_tier` (`admin.rs:2441`) | warm | enum `ContractTier` (`state/perp_market.rs:80-94`) | **unchecked** |
| `State.initial_pct_to_liquidate` | `update_initial_pct_to_liquidate` (`admin.rs:2624`) | warm | `LIQUIDATION_PCT_PRECISION` (1e4, `constants.rs:46`) | **unchecked** (values above 10000 are inert: the sum is min'd at `math/liquidation.rs:440`) |
| `State.liquidation_duration` | `update_liquidation_duration` (`admin.rs:2638`) | warm | 400ms units, u8 (`StoredSlotDuration<u8, 400>`) | **unchecked** (0 is safe by fallback: `math/liquidation.rs:437-438` defaults to 100% freeable) |
| `State.liquidation_margin_buffer_ratio` | `update_liquidation_margin_buffer_ratio` (`admin.rs:2653`) | warm | `MARGIN_PRECISION`; default 2% (`constants.rs:357`) | **unchecked** |
| `User.equity_floor` | `update_user_equity_floor` (`admin.rs:4824`) | warm | `QUOTE_PRECISION`; 0 disables | **unchecked** |
...
at the next keeper evaluation, with no grace period. Both bite immediately, and nothing is latched.
...
imbalances* setter (`admin.rs:1606-1616`). There are two hazards. The change re-prioritizes
liquidation ordering for every open position immediately. And a downgrade leaves the previously-set
insurance caps above the new tier's limit (the speculative tiers cap insurance at 0,
`constants.rs:201`) with no error and no event, until an admin separately re-runs the
max-imbalances setter. Nothing re-validates the pair.
...
does not decide who is liquidatable, since entry is unbuffered. It sets how deep a liquidation goes
...
every subaccount to clear floor + buffer (`admin.rs:4803-4811`). The full lifecycle is in
[docs/EQUITY-FLOOR.md](./EQUITY-FLOOR.md).

One doc comment misleads operators. The field docs of `PerpMarket.imf_factor` and
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `SpotMarket.initial_asset_weight` | `update_spot_market_margin_weights` (`admin.rs:2068`) | warm | `SPOT_WEIGHT_PRECISION` (1e4) | market 0: exactly 10000; others: <= 10000 and <= maintenance weight; 0 allowed (`validation/margin.rs:64-69,92-106`) |
...
| `SpotMarket.optimal_utilization` | `update_spot_market_borrow_rate` (`admin.rs:2130`) | warm | `SPOT_UTILIZATION_PRECISION` (1e6) | <= 1_000_000 (`validation/spot_market.rs:17-22`) |
...
| `SpotMarket.min_borrow_rate` | same handler | warm | u8, 1 unit = 0.5% (`state/spot_market.rs:197-200`) | expanded (u8 x 5000) <= optimal rate; re-validated even when unchanged (`admin.rs:2145`, `validation/spot_market.rs:32-38`) |
| `SpotMarket.withdraw_guard_threshold` | `update_withdraw_guard_threshold` (`admin.rs:1865`) | warm | token mint precision | oracle live price and 5-min TWAP both > 0; notional at max(live, TWAP) <= $10,000 (`admin.rs:1885-1898`, `validation/spot_market.rs:46-74`, cap `constants.rs:373`) |
| `SpotMarket.withdraw_circuit_breaker_bps` | `update_spot_market_withdraw_circuit_breaker` (`admin.rs:2207`) | warm; **cold to exceed 25%** | bps (10000 = 100%) | <= 10000 for anyone; <= 2500 unless the signer is cold (`admin.rs:2211-2231`) |
| `SpotMarket.max_token_deposits` | `update_spot_market_max_token_deposits` (`admin.rs:2187`) | warm | token mint precision; 0 = no limit | **unchecked** |
| `SpotMarket.max_token_borrows_fraction` | `update_spot_market_max_token_borrows` (`admin.rs:2276`) | warm | fraction of `max_token_deposits`, x/10000 | derived cap must cover current outstanding borrows (`admin.rs:2289-2301`); the fraction itself may exceed 100% |
| `SpotMarket.deposit_guard_threshold` | `update_spot_market_deposit_cap` (`admin.rs:2249`) | warm | token mint precision | **unchecked** |
| `SpotMarket.max_deposit_bps_per_day` | same handler | warm | bps/day; 0 disables the limiter | **unchecked** (u16 caps it at 655%/day) |
| `SpotMarket.scale_initial_asset_weight_start` | `update_spot_market_scale_initial_asset_weight_start` (`admin.rs:2310`) | warm | `QUOTE_PRECISION` notional; 0 disables | **unchecked** |
| `SpotMarket.insurance_fund.unstaking_period` | `update_insurance_fund_unstaking_period` (`admin.rs:1785`) | warm | seconds (i64); default 13 days (`constants.rs:124`) | **unchecked** |
| `SpotMarket.insurance_fund.revenue_settle_period` | `update_spot_market_revenue_settle_period` (`admin.rs:1973`) | warm | seconds (i64) | > 0 only (`admin.rs:1980`) |
| `SpotMarket.insurance_fund.if_fee_factor` | `update_spot_market_if_factor` (`admin.rs:1915`) | warm | `IF_FACTOR_PRECISION` (1e6) | `if_fee_factor + protocol_fee_factor` strictly < 100% (`admin.rs:1937-1941`) |
| `SpotMarket.protocol_fee_factor` | same handler | warm | `IF_FACTOR_PRECISION` | same joint bound; no individual cap |
| `SpotMarket.if_paused_operations` | `update_spot_market_if_paused_operations` (`admin.rs:2350`) | pause | u8 bitmask of `InsuranceFundOperation` (`state/paused_operations.rs:88-94`) | pause admin may only add bits (`admin.rs:2357-2362`, `auth.rs:112-122`); warm/cold set anything |
...
the same way the perp maintenance ratio does. One coupling is enforced in only one direction.
`update_spot_market_asset_tier` requires the tier to be Collateral or Protected when
`initial_asset_weight > 0` (`admin.rs:2040-2063`), but this weights handler can raise the weight
...
`calculate_borrow_rate` (`math/spot_balance.rs:385-431`). The optimal point is the pivot between
the linear low-utilization slope and the hard-coded high-utilization segments
(`constants.rs:379-386`), `max_borrow_rate` is the rate at 100% utilization, and the expanded
`min_borrow_rate` floors the result at any utilization. `optimal_utilization` also feeds the
withdraw/borrow utilization ceiling (`math/spot_withdraw.rs:236`), and at the permitted extreme of
100% it disables that half of the limiter. The timing is what makes this dangerous. Interest
accrual is computed over the whole interval since `last_interest_ts` in one shot
(`math/spot_balance.rs:297-345`), so a rate change applies retroactively to all un-accrued elapsed
time, and there is no absolute ceiling on `max_borrow_rate`. One warm-admin write followed by any
instruction that touches the market can bill every existing borrower at an extreme rate for the
elapsed span.
...
cap (`admin.rs:5136-5140`). `withdraw_circuit_breaker_bps` sets how much of the 24h deposit TWAP
may leave per day (`math/spot_withdraw.rs:21-47`). 0 is a sentinel for the default 25% rather than
a freeze, and only the cold admin can loosen past 25%. Lowering it can block a withdrawal that
...
freeze the whole accrual block (the comment at `admin.rs:1931-1936` spells this out). The same
...
(`auth.rs:60-63` gives the reason, which is that an oracle swap re-prices the withdraw-guard
notional cap and every other rail). The staleness and validity thresholds, by contrast, all live in
one `State` struct written by a single instruction.

| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `SpotMarket.oracle` + `oracle_source` | `update_spot_market_oracle` (`admin.rs:1027`) | **cold** | Pubkey + `OracleSource` enum | account must decode under the source; new price > 0 and within 10% of the old (`admin.rs:1037-1104`), unless `skip_invariant_check=true`; four Pyth-pull variants rejected (`admin.rs:105-114`); see caveats below |
| `PerpMarket.oracle` + `oracle_source` | `update_perp_market_oracle` (`admin.rs:2928`) | **cold** | same | same checks (`admin.rs:2940-3011`), same caveats |
| `State.oracle_guard_rails.*` (all 6 fields) | `update_oracle_guard_rails` (`admin.rs:2684`) | warm | see below | the two staleness windows are range-checked, `slots_before_stale_for_amm` to 0..=127 and `slots_before_stale_for_margin` to 0..=1_000_000 stored units (`admin.rs:2686-2698`); the other four fields are **unchecked** |
| `PerpMarket.oracle_low_risk_slot_delay_override` | `update_perp_market_oracle_low_risk_slot_delay_override` (`admin.rs:3329`) | warm | 400ms units (i8), decoded by `DelayOverride`; 0 = fall back to state | **unchecked** |
| `PerpMarket.oracle_slot_delay_override` | `update_perp_market_oracle_slot_delay_override` (`admin.rs:3349`) | warm | 400ms units (i8); negative = unset; 0 disables immediate AMM fills | **unchecked** |
| `PrelaunchOracle.price` / `max_price` | `update_prelaunch_oracle_params` (`admin.rs:3544`) | warm | `PRICE_PRECISION` (1e6) | both != 0 and `price <= max_price` (`state/oracle.rs:758-772`); no sign or magnitude bound |
| `State.feature_bit_flags` MmOracleUpdate bit | `update_feature_bit_flags_mm_oracle` (`admin.rs:4503`) | hot (`HotRole::FeatureFlag`) to disable; **cold to re-enable** (`admin.rs:4509-4513`) | one bit | single-bit read-modify-write; other bits undisturbed |

### Oracle swaps (cold)

Both swap handlers enforce price continuity (new price > 0, within 10% of the old,
`admin.rs:1096-1104`) unless the admin passes `skip_invariant_check=true`, which reduces the check
...
changes two behaviors with no error and no event. It triples the margin-staleness budget
(`math/oracle.rs:479-486`) and snaps prices within 5 bps to exactly $1
...
### Oracle guard rails (warm)

One instruction assigns the whole struct. It range-checks the two staleness windows and writes the
other four fields with no validation. What each field gates, and what the read sites impose that
the setter does not:
...
- `price_divergence.mark_oracle_percent_divergence` (`PERCENTAGE_PRECISION`, default 10%).
  ...
  gate (`math/oracle.rs:268,356-359`). It is not on the taker fill path. The reader floors the
  effective value at 10% (`math/oracle.rs:26-33`). No ceiling exists, so a large value disables
...
- `price_divergence.oracle_twap_5min_percent_divergence` (default 50%). Aborts fills, triggers,
  ...
  the floor it can already block liquidation of underwater users on a fast-moving market. There is
  no ceiling, and values above i64::MAX fail as `CastingFailure` at read time.
- `validity.slots_before_stale_for_amm` (i64 in 400ms stored units, default 10, i.e. 4s). The
  default AMM low-risk staleness threshold wherever the per-market override is 0
  (`math/oracle.rs:472-477`), which is all spot markets and any perp market left at init default.
  `get_oracle_status` narrows the value to i8 before passing it to `oracle_validity`
  (`math/oracle.rs:332-333`), so a value outside i8 range would make every `update_funding_rate`
  revert with `CastingFailure` and halt funding accrual protocol-wide. The setter forecloses that.
  `MAX_AMM_STALENESS_STORED_UNITS` is `i8::MAX` and the accepted range starts at 0, so neither an
  out-of-range value nor a negative one (which would freeze AMM low-risk fills even on a same-slot
  price) can be written (`admin.rs:2678-2698`).
- `validity.slots_before_stale_for_margin` (i64 in 400ms stored units, default 120, i.e. 48s).
  The setter caps it at `MAX_STALENESS_STORED_UNITS` = 1,000,000 units (~4.6 days) and rejects
  negatives (`admin.rs:2670-2698`). Past the threshold, oracles are
  ...
  (`math/oracle.rs:209-237`), so lowering it does not block liquidating a stale-priced account.
- `validity.confidence_interval_max_size` (`BID_ASK_SPREAD_PRECISION`, default 2%). Multiplied
  ...
  (`math/oracle.rs:412-416`). Exceeding it yields `TooUncertain`, which rejects fills, margin
  validity, and PnL settlement, while liquidations and triggers tolerate it. 0 blocks all of the
  former while still allowing liquidations.
- `validity.too_volatile_ratio` (unitless i64, default 5). Trips `TooVolatile` when
  ...
  (`math/oracle.rs:405-407`). `TooVolatile` rejects everything except TWAP and AMM-curve
  updates, so it halts the market. Any value <= 0 trips on every read with a positive price and
  freezes the market entirely. 0 is also the struct's `#[derive(Default)]` zero-value.
...
semantic checks. There is no per market configuration or mutable baseline that a compromised hot
key could widen.
...
AMM threshold when nonzero, and every negative value clamps to a threshold of 0 (same-slot price
required), which blocks AMM low-risk fills outright. `oracle_slot_delay_override` gates
immediate (JIT, auction-skipping) AMM fills. 0 disables them for the market. A negative value
means unset, which resolves to `MM_ORACLE_MIN_WRITE_GAP` (800ms) for MM-sourced prices
(`constants.rs:389-392`). A high positive value lets JIT fills execute against a price up to 127
units (50.8s) old, plus up to 800ms of hidden MM-oracle source age
(`constants.rs:403-413`), which exposes counterparties to stale-price arbitrage. These controls remain warm-only and are deliberately excluded from the vAMM
...
overwrites the market's mark TWAPs (`admin.rs:3559-3569`), moving every open position's
...
A negative price passes `validate()` but freezes the market rather than mis-pricing it, because
every action rejects a `NonPositive` oracle (`math/oracle.rs:403,488-489`).

### MM-oracle kill switch

Clearing the `MmOracleUpdate` bit stops both native MM-oracle crank handlers from storing new
prices (`admin.rs:3962-3965,4256-4259`). Positions do not freeze. Pricing falls back to the
exchange oracle once the stored MM price ages past it (`state/oracle.rs:333-357`), and
immediate-fill staleness tightens. Disabling needs only the `FeatureFlag` hot key, and re-enabling
requires the cold admin exactly. The MM-oracle write path itself (step caps, write gaps) is
governed by hard-coded constants, not admin parameters (`constants.rs:388-413`).

## Market status, pauses, and kill switches

These setters span the widest range of authority tiers in the program. The exchange-wide and
per-market pause bitmasks admit the pause admin (add-only), feature kill switches sit on a hot key
(disable-only, with cold-only re-enable), one circuit breaker is fully permissionless, and the
solvency switch is cold-only.

| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `State.exchange_status` | `update_exchange_status` (`admin.rs:3432`) | pause | u8 bitmask, 8 `ExchangeStatus` bits (`state/state.rs:188-199`) | pause admin add-only (`admin.rs:3438`); warm/cold set anything; all 8 bits defined |
| `PerpMarket.status` | `update_perp_market_status` (`admin.rs:2373`) | warm | `MarketStatus` enum | Settlement and Delisted rejected (`admin.rs:2377-2381`); no transition checks within {Initialized, Active, ReduceOnly} |
| `SpotMarket.status` | `update_spot_market_status` (`admin.rs:1993`) | warm | `MarketStatus` enum | **unchecked**: any of the 5 variants, Delisted included; see below |
| `PerpMarket.paused_operations` | `update_perp_market_paused_operations` (`admin.rs:2400`) | pause | u8 bitmask, 8 `PerpOperation` bits (`state/paused_operations.rs:6-16`) | three-way: cold anything; warm only the `UpdateFunding`/`SettleRevPool` bits (`admin.rs:2412-2422`); pause admin add-only (`admin.rs:2423-2428`) |
| `SpotMarket.paused_operations` | `update_spot_market_paused_operations` (`admin.rs:2013`) | pause | u8 bitmask, 5 `SpotOperation` bits (`state/paused_operations.rs:57-64`) | pause admin add-only (`admin.rs:2022-2027`); warm/cold anything; undefined bits 5-7 stored but inert |
...
| `SpotMarket.expiry_ts` + status | `update_spot_market_expiry` (`admin.rs:1117`) | warm | Unix seconds | expiry must be in the future (`admin.rs:1125-1129`); status hard-coded to ReduceOnly (`admin.rs:1143`) |
| `PerpMarket.expiry_ts` + status | `update_perp_market_expiry` (`admin.rs:1152`) | warm | Unix seconds | expiry must be in the future (`admin.rs:1160-1164`); status hard-coded to ReduceOnly (`admin.rs:1178`) |
| `SpotMarket.orders_enabled` | `update_spot_market_orders_enabled` (`admin.rs:2330`) | warm | bool | **unchecked**, and dead: no program code reads it (see below) |
| `State.solvency_status` | `update_solvency_status` (`admin.rs:3448`) | **cold** | u8, only bit 0 defined (`state/state.rs:208-216`) | **unchecked**: any u8 stored; values >= 2 brick the readers (see below) |
| `UserStats.paused_operations` | `admin_update_user_stats_paused_operations` (`admin.rs:3491`) | pause (or hot `UserFlag`) | u8 bitmask, 3 bits (`state/user.rs:2176-2183`) | pause admin add-only (`admin.rs:3505-3511`); cold/warm/hot-UserFlag set anything |
| `State.feature_bit_flags`: MedianTriggerPrice, BuilderCodes | `update_feature_bit_flags_*` (`admin.rs:4524,4545`) | hot (`FeatureFlag`) to disable; **cold to enable** | one bit each (`state/state.rs:606-612`) | single-bit read-modify-write; asymmetric authority in-handler (`admin.rs:4530-4534,4551-4555`) |
| `State.lp_pool_feature_bit_flags`: SettleLpPool, SwapLpPool, MintRedeemLpPool | `update_feature_bit_flags_*_lp_pool` (`admin.rs:4587,4608,4629`) | hot to disable; cold to enable | one bit each (`state/state.rs:614-619`) | same asymmetry; consumers compiled out of mainnet builds (`vlp-hedge` feature) |
| `PerpMarket.hedge_config.paused_operations` | `update_perp_market_lp_pool_paused_operations` (`admin.rs:3307`) | pause | u8 bitmask, 2 `PerpLpOperation` bits (`state/paused_operations.rs:117-121`) | pause admin add-only (`admin.rs:3315-3320`); no Delisted gate |
...
The pause admin can only add bits, and unpausing needs warm or cold.

Perp market status. ReduceOnly forces every fill in the market to be risk-reducing
(`controller/orders.rs:175,1106,1675`), and positions can still be closed. Initialized is much
...
Spot market status carries the largest unguarded consequence in this section. The handler writes
any status with no validation, including Delisted. Withdrawals admit only
Active|ReduceOnly|Settlement (`controller/spot_position.rs:157-165`), so writing Delisted (or
Initialized) blocks all withdrawals in the market. Every writer of spot status carries
`#[access_control(spot_market_valid)]`, which rejects a Delisted market, so nothing can write the
status back afterwards. One warm-tier transaction can therefore permanently strand every user's
...
and one window needs care. Between `expiry_ts` and a keeper running `settle_expired_market`,
...
refuses liquidations (leaving underwater positions un-liquidatable until cleared), and the AMM bits
...
IF unstaking, and the two legs of spot liquidations, and there is no warm bit budget on the spot
side. The permissionless `pause_spot_market_deposit_withdraw` can OR in the Deposit and Withdraw
bits, but only when the spot vault invariant is genuinely violated onchain. Any signer can trip
...
the pause admin (`admin.rs:2415`). A single key holding both the warm and pause roles therefore
...
`instructions/keeper.rs:2091-2094,2257-2260,2406-2409`), and a bankrupt user stays bankrupt until
it clears. The missing bound can lock the admin out of its own repair path. The readers decode the
byte with `BitFlags::from_bits(...)` and only bit 0 is defined, so any stored value >= 2 makes all
three repair instructions fail with `FailedUnwrap` until the cold admin writes 0 or 1 back
(`state/state.rs:444-445`, `math/safe_unwrap.rs:32-47`). Other bitmask setters guard against
...
warm/cold) can *disable*, and only the cold admin can *re-enable*. `MedianTriggerPrice` changes
...
share one bit, so there is no way to close entry while leaving the exit open. The LP-pool
...
One switch is dead. `SpotMarket.orders_enabled` has no reader anywhere in the program. The
declaration, default, initializer, and setter are the only references. The SDK admin wrapper's doc
comment (`packages/sdk/src/adminClient.ts:3776`) claims it enables or disables spot limit orders.
Onchain it does nothing. Spot DLOB trading is disabled by a hard-coded rejection instead
(`controller/orders.rs:835-843`).

LP-pool hedge config on mainnet. `hedge_config.status` (unchecked, warm) kills both LP-pool
revenue tracking and quote-owed settlement for a market when 0. The loops skip rather than revert,
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `PerpMarket.order_step_size` | `update_perp_market_step_size_and_tick_size` (`admin.rs:3030`) | warm | `BASE_PRECISION` (1e9) | 1..=2_000_000_000 (`admin.rs:3038-3039`); no divisibility re-check against existing state (see below) |
...
| `SpotMarket.pool_id` | `update_spot_market_pool_id` (`admin.rs:474`) | warm | u8 pool tag (0 = main, 2 = LST, `constants.rs:376`) | value **unchecked**; market status must be Initialized (`admin.rs:485-489`) |
| `PerpMarket.hedge_config.pool_id` | `update_perp_market_lp_pool_id` (`admin.rs:1766`) | warm | u8 LP-pool id | **unchecked**; no cross-check against any LPPool account |
...
three concrete failure modes. Post-only Slide placement underflows (`math/orders.rs:333,339`), the
AMM fill-sizing helper underflows (`math/orders.rs:176`), and oracle-offset limit prices are
floored at one tick with no error (`state/user.rs:1543`).
...
(`state/perp_market.rs:1001-1008`, where OI/1000 is clamped into [100x, 5000x] of it) and AMM fallback
...
hard-fails the whole LP-pool hedge settle batch (`vlp/hedge/settle.rs:87-95`) rather than skipping the market.
...
one quirk worth knowing if spot trading is ever re-enabled. The quote market (index 0) is exempt
from the > 0 checks, so 0 is writable for it, which would show up as `MathError`s in the sizing
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `State.perp_fee_structure` (10 fee tiers, filler rewards, AMM/IF split) | `update_perp_fee_structure` (`admin.rs:2592`) | warm | tier fees in tenths of a bp (numerator over denominator >= `FEE_DENOMINATOR` 100_000, `constants.rs:210`); filler/referral in percent; `flat_filler_fee` in `QUOTE_PRECISION` | taker fee <= 30 bps; maker rebate <= 3 bps; referee/referrer <= 100%; filler reward <= 20%; `flat_filler_fee` <= $0.01; `amm + if` numerators <= 100; two derived consistency checks (fill must leave the market at least the maker rebate) (`validation/fee_structure.rs:15-137`). No lower bounds: an all-zero schedule is legal |
| `State.spot_fee_structure` | `update_spot_fee_structure` (`admin.rs:2608`) | warm | same | same validator; almost entirely inert (see below) |
| `PerpMarket.fee_adjustment` | `update_perp_market_fee_adjustment` (`admin.rs:3170`) | warm | signed percent of the base fee, i16 | -100..=100 (`admin.rs:3177-3183`, `FEE_ADJUSTMENT_MAX` `constants.rs:250`) |
| `PerpMarket.taker_fee_addon_tenth_bps` | `update_perp_market_taker_fee_addon` (`admin.rs:3195`) | warm | tenth-bps (u16, unsigned: surcharge only); field `state/perp_market.rs:307` | <= 100 tenth-bps = 10 bps (`admin.rs:3202-3207`, `MAX_TAKER_FEE_ADDON_TENTH_BPS` `constants.rs:234`) |
| `State.promo_fee_tier` | `update_promo_fee_tier` (`admin.rs:2540`) | warm | perp fee-tier index (u8); 0 disables | <= highest populated tier, `PERP_FEE_TIER_MAX_INDEX` = 3 (`admin.rs:2550-2555`, `constants.rs:240`) |
| `SpotMarket.fee_adjustment` | `update_spot_market_fee_adjustment` (`admin.rs:3369`) | warm | same as perp `fee_adjustment` | same bound; no reader exists (see below) |
| `PerpMarket.fee_pool_buffer_target` | `update_perp_market_fee_pool_buffer_target` (`admin.rs:3220`) | warm | `QUOTE_PRECISION` | **unchecked** |
| `PerpMarket.bankruptcy_if_floor_pct` | `update_perp_market_bankruptcy_if_floor_pct` (`admin.rs:3244`) | warm | `PERCENTAGE_PRECISION` (u32); 0 disables | <= 1_000_000 = 100% of OI notional (`admin.rs:3251-3255`) |
| `State.feature_bit_flags` VammMakerRebate bit | `update_feature_bit_flags_vamm_maker_rebate` (`admin.rs:4566`) | hot (`FeatureFlag`) to disable; **cold to enable** (`admin.rs:4572-4576`) | one bit | single-bit read-modify-write |
| `State.protocol_fee_recipient_perp` / `_spot` | `update_protocol_fee_recipient` (`admin.rs:5348`) | **cold** | Pubkey | **unchecked** (any 32 bytes, `Pubkey::default()` included) |

### What a change does

Perp fee structure. Rewrites the global schedule read on every perp fill
(`controller/orders.rs:1422` into `math/fees.rs:78,384`). The schedule sets the taker fee, maker
rebate, filler rewards, and the AMM/IF/protocol split of the remainder. A resting maker order or
in-flight taker order is priced under whichever schedule is live at the instant it fills, and
nothing is grandfathered. `flat_filler_fee` also sets the keeper reward for expired, triggered, and
...
trailing-30d volume, `math/fees.rs` `VOLUME_THRESHOLDS`), evaluated at every fill against a
live-decayed volume projection (`state/user.rs:2105`), so schedule changes bite at the account's
next fill.
...
buffer, and it is capped at 10 bps. It is read live, so a resting taker order pays the add-on in
force at the instant it fills.
...
because the tier function clamps. An unbounded value would pass validation and then resolve to a
lower tier with no error (the handler comment at `admin.rs:2546-2549` records this).
...
entitlement. An arbitrarily large value stalls the IF sweep and AMM-provision tokenization for the
market with no error, because the subtraction saturates, and it does so even at delisting. It is
also the one fee setter with no Delisted-market gate. See FEES.md's sweep-buffer discussion.

Bankruptcy IF floor sizes the standing first-loss tranche the sweeps must leave backed, computed
as `open_interest x oracle TWAP x pct` (`state/perp_market.rs:938-982`) and reserved at
...
feature bits. The hot key can turn it off, and only cold can turn it on.
...
that the recipient can actually sign, so a mistyped authority still receives tokens into a freshly
created ATA it may never be able to move.

The spot fee setters are inert. Spot DLOB trading is deleted in this fork. There are no spot fill
instructions, and placement is rejected at `controller/orders.rs:835-843`, so the spot fee tiers,
...
(`controller/orders.rs:4118`). An operator changing spot fees reprices nothing, which is the
hazard worth stating.

## State-level globals, funding, and admin keys

This section collects the parameters that do not fit the earlier groupings. It contains both the
best-behaved gate in the program (the PnL-pool credit, which is solvency-checked after the fact)
and the worst single missing bound found in this review (the funding period).

| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `PerpMarket.market_stats.funding_period` | `update_perp_market_funding_period` (`admin.rs:1530`) | warm | seconds (i64) | >= 0 only (`admin.rs:1541`); downstream math imposes a hidden functional ceiling (see below) |
| `PerpMarket.funding_clamp_threshold` | `update_perp_market_funding_dead_zone` (`admin.rs:1556`) | warm | bps (`BPS_PRECISION` 10000) | < 10000, strict (`admin.rs:1569-1572`) |
| `PerpMarket.funding_ramp_slope` | same handler | warm | multiplier at `PERCENTAGE_PRECISION`; default 1.0x | > 0 only (`admin.rs:1574`); no upper cap (u32, up to ~4294x) |
| `PerpMarket.pnl_pool` (credit) | `update_perp_market_pnl_pool` (`admin.rs:1371`) | warm | quote token amount | no numeric cap, but the credit must leave the vault covering all depositor claims (`admin.rs:1387`, `math/spot_withdraw.rs:532-547`); no tokens move |
| `PerpMarket.name` | `update_perp_market_name` (`admin.rs:1666`) | warm | [u8; 32] | **unchecked** |
| `SpotMarket.name` | `update_spot_market_name` (`admin.rs:1679`) | warm | [u8; 32] | reserved name "USDT" only on market 0 (`admin.rs:1684-1689`); otherwise anything |
| `SpotMarket.asset_tier` | `update_spot_market_asset_tier` (`admin.rs:2040`) | warm | `AssetTier` enum (5 variants, `state/spot_market.rs:750-762`) | must be Collateral/Protected while `initial_asset_weight > 0` (`admin.rs:2047-2053`); otherwise any variant |
| `State.settlement_duration` | `update_state_settlement_duration` (`admin.rs:2710`) | warm | seconds (u16) | **unchecked** (0 to ~18.2h) |
| `State.max_number_of_sub_accounts` | `update_state_max_number_of_sub_accounts` (`admin.rs:2897`) | warm | count (u16); values > 5 mean value x 100 (`state/state.rs:454-460`); 0 = unlimited | **unchecked** |
| `State.max_initialize_user_fee` | `update_state_max_initialize_user_fee` (`admin.rs:2911`) | warm | hundredths of 1 SOL (u16) | **unchecked** (up to 655.35 SOL) |
| `PerpMarket.number_of_users` / `number_of_users_with_base` | `update_perp_market_number_of_users` (`admin.rs:3268`) | warm | counts (u32) | only `users >= users_with_base` (`admin.rs:3298-3302`); no check against the true count; works on any market status |
| `PerpMarket.market_config` (DisableFormulaicKUpdate bit) | `update_perp_market_config` (`admin.rs:4653`) | warm to clear; **cold-key to set** (`admin.rs:4669-4675`) | u8, 1 defined bit | unknown bits rejected (`admin.rs:4659-4664`) |
| `User.special_user_status` (VammHedger bit) | `update_special_user_status` (`admin.rs:4688`) | hot (`UserFlag`) to clear; **cold-key to set** (`admin.rs:4703-4709`) | u8, 1 defined bit | unknown bits rejected (`admin.rs:4694-4699`) |
| `State.discount_mint` | `update_discount_mint` (`admin.rs:3418`) | warm | Pubkey | **unchecked**, and no reader exists (vestigial) |
| `State.whitelist_mint` | `update_whitelist_mint` (`admin.rs:3404`) | none: **not callable** | Pubkey | the lib.rs wrapper is commented out (`lib.rs:1691-1696`); the field is frozen at `Pubkey::default()` (whitelist off) |
| `State.cold_admin` | `update_admin` (`admin.rs:3394`) | **cold** | Pubkey | **unchecked** (`Pubkey::default()` accepted) |
| `State.warm_admin` | `update_warm_admin` (`admin.rs:5309`) | **cold** | Pubkey | **unchecked** |
| `State.pause_admin` | `update_pause_admin` (`admin.rs:5319`) | **cold** | Pubkey | **unchecked** |
| `State.hot_<role>` (13 roles) | `update_hot_admin` (`admin.rs:5333`) | warm | Pubkey per `HotRole` (`state/state.rs:170-186`) | **unchecked** |

### What a change does

Funding period is the worst missing bound in the program. The setter checks only `>= 0`, but
the value is read on *every fill*, because funding updates run at the end of every trade
(`controller/orders.rs:1541-1550`). The downstream math has two revert bands. Above 86,400s
the period adjustment integer-divides to 0 and the next due funding update returns `MathError`
(`controller/funding.rs:361-391`). Above i64::MAX/2 the schedule helper overflows on every
call (`math/helpers.rs:87`). Either way the error propagates through the fill path, so every
fill in the market fails until the value is corrected. 0 is also legal. It collapses
the mark-TWAP window to the last print (`math/stats.rs:87`), which defeats the mark/oracle
divergence rails, and it divides by zero in the median-trigger-price basis when that feature is on
(`state/perp_market.rs:1182-1195`). A larger period also *raises* the per-interval funding rate,
because the period adjustment is a divisor. That is the reverse of the intuition that a longer
period means a gentler one.

Funding dead zone and ramp. The clamp threshold carves a no-premium band around the oracle
TWAP (`controller/funding.rs:374-377`, `math/funding.rs:79-97`), and widening it can zero the
...
PnL pool credit. This is not a token deposit. It re-attributes existing quote-vault surplus into
...
Asset tier also sets the market's oracle tolerance, which is easy to miss. Beyond gating borrows
(Protected bans them, `controller/spot_position.rs:174-179`) and capping isolated liabilities
(`math/margin.rs:668-705`), the tier keys the market's accepted oracle confidence multiplier at 1x
for Collateral/Protected, 5x for Cross, and 50x for Isolated/Unlisted
(`state/spot_market.rs:405-412`). It also keys the TWAP sanitize clamp
(`state/spot_market.rs:414-422`).
...
(`state/state.rs:462-481`), and any nonzero value also gates deletion of fresh, non-idle
...
(`admin.rs:925-932`) and the final delist sweep (`admin.rs:1225-1236`). Overriding
...
at 0 with the true count unrecoverable onchain. The handler carries no market-status gate.
...
vAMM position-transfer instruction (`instructions/user.rs:4687-4691`; the consumer tests
exact equality, so a hypothetical second bit would revoke access with no error). Both share a
...
the protection, not any value check. Cold rotates itself, warm, and pause. Warm rotates the 13
hot-role keys. Rotations bite at the very next authority check with no timelock (the module doc
`auth.rs:1-14` places the timelock in off-chain key custody). Two asymmetries are worth knowing.
Setting `warm_admin` or a hot key to `Pubkey::default()` is recoverable, because the tier collapses
onto cold. Setting `cold_admin` to `Pubkey::default()` or an unrecoverable key permanently
disables every cold-gated path, including the rotation handler itself, oracle swaps, and the
ability to ever set the cold-gated config bits. Nothing onchain guards against it.

Dead or inert. `State.discount_mint` has no reader, an upstream fee-discount leftover like
`State.srm_vault`. `State.whitelist_mint` cannot be changed at all, because the instruction wrapper
is commented out of `lib.rs`, so the deposit whitelist is permanently off absent a program upgrade.
Market names have no onchain reader. The one enforced rule, that only market 0 may be named
"USDT", exists because off-chain monitoring keys its stablecoin exemption off the decoded name
(`admin.rs:180-189`). Perp names carry no such rule, so a perp rename can still mislead
dashboards.
...
re-checked by a second pass that actively tried to find a validator. This document stays
descriptive. Whether any of these deserves a code change is a separate decision. They are ordered
by blast radius.

1. `funding_period` can halt all fills in a market (`admin.rs:1541` checks only `>= 0`).
...
2. Spot `MarketStatus::Delisted` is an irreversible withdrawal freeze. The status setter
   accepts any variant unchecked (`admin.rs:1993-2008`), withdrawals reject Delisted
...
3. Spot `liquidator_fee` has no relationship to the liability weights
   (`admin.rs:1817-1824` bounds only the three-fee sum below 100%). A fee above the market's
...
4. Four of the six oracle guard rails are written with no validation (`admin.rs:2684-2698` bounds
   only `slots_before_stale_for_amm` and `slots_before_stale_for_margin`). `too_volatile_ratio <= 0`
   freezes the market on every read, and 0 is the struct's derive-default. The two divergence fields
   have read-side floors but no ceilings, so large values disable the bands. The staleness windows
   used to be unbounded too, and an out-of-i8-range `slots_before_stale_for_amm` would have bricked
   every funding update through the cast at `math/oracle.rs:332-333`. The setter now rejects that
   range, so this one is closed.
5. `update_solvency_status` accepts bytes its own readers cannot decode. Only bit 0 is
   defined, so any stored value >= 2 makes all three solvency-repair instructions fail with
   `FailedUnwrap` until rewritten (`state/state.rs:444-445`). The unknown-bit guard used by
   `market_config` and `special_user_status` (`admin.rs:4659-4664,4694-4699`) is missing here.
6. `contract_tier` is unchecked and desynchronizes the insurance caps (`admin.rs:2441`).
...
7. Perp `order_step_size` is not re-checked against existing state (`admin.rs:3030`). A
...
8. `max_borrow_rate` has no absolute ceiling and applies retroactively (u32 domain reaches
   ...
   (`math/spot_balance.rs:297-345`), one write can bill every existing borrower at an extreme
   rate for elapsed time.
9. `insurance_fund.unstaking_period` is a fully unchecked signed value (`admin.rs:1785`),
   ...
   instantly, and a huge value strands them all.
10. `liquidation_margin_buffer_ratio` is an uncapped u32 (`admin.rs:2653`). Large values
...
11. `scale_initial_asset_weight_start` treats 1 and 0 very differently (`admin.rs:2310`,
...
12. `deposit_guard_threshold` lacks the $10k notional cap its withdraw-side mirror carries
    (`admin.rs:2249` vs `validation/spot_market.rs:46-74`), so the deposit rate limiter can be
    disabled by one warm write.
13. `fee_pool_buffer_target` is unchecked and stalls sweeps with no error (`admin.rs:3220`). An
...
14. `SpotMarket.pool_id` can be repointed on a live market (`admin.rs:474`). The
...
15. `cold_admin` accepts `Pubkey::default()` (`admin.rs:3394`). That write permanently
...
- Key collisions defeat the tier matrix. A key that is both warm and pause admin bypasses
  the add-only rule and the perp pause bit budget (`auth.rs:118`, `admin.rs:2415`). The
...
- "Unchecked" here means unchecked onchain. The admin CLI (`packages/cli-admin/`), the
...
- `packages/cli-admin/README.md`, the operator commands that drive these instructions.
