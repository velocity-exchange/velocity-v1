...
impl<'a> MatchFulfillmentParams<'a> {
    ...
    ) -> VelocityResult<MatchFulfillmentParams<'a>> {
...
impl<'a> SpotFulfillmentParams for MatchFulfillmentParams<'a> {
    ...
    fn get_best_bid_and_ask(&self) -> VelocityResult<(Option<u64>, Option<u64>)> {
    ...
    ) -> VelocityResult<ExternalSpotFill> {
        Err(ErrorCode::InvalidSpotFulfillmentParams)
    }

    fn get_order_action_explanation(&self) -> VelocityResult<OrderActionExplanation> {
        Err(ErrorCode::InvalidSpotFulfillmentParams)
    }
