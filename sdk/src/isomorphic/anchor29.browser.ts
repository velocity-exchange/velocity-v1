...
export class AnchorProvider {
	constructor() {
		throw new Error(
			'Anchor (0.29) is not supported in the browser build. Use `DriftCore` instead of `VelocityClient`.'
		);
	}
}
...
export class Program<_T = any> {
	constructor() {
		throw new Error(
			'Anchor (0.29) Program is not supported in the browser build. Use `DriftCore` instead of `VelocityClient`.'
		);
	}
}
