...
How the equity floor on delegated accounts works. This covers what it enforces, what happens when
it trips, and how to move funds between subaccounts without tripping it.
...
"Equity" here is the subaccount's net equity, meaning unweighted asset value, plus funding-inclusive
unrealized PnL, minus unweighted spot liability value, all at live oracle prices. The per-action
gates compute it with `calculate_net_equity_for_floor` and the SDK mirrors it with
`User.getFloorNetEquity()`. This is what the account is actually worth rather than the weighted
margin numerator. Borrows subtract their full value, and deposits and PnL count unweighted and
unclamped. Every floor check and the breaker trip use this one metric.
...
5-minute TWAP can share the same stale value, so no stored price can bracket the true one.

Five kinds of gate authorize an action: withdrawals, risk-increasing placement and fills, transfers
out, trigger-order activation, and liquidator admission. Each of them rejects with `InvalidOracle`
while any oracle the subaccount's positions depend on is invalid. Each rejects with
`EquityBelowFloor` when a fully valid value sits below `floor + buffer`. A bad price can therefore
never authorize an action through the floor.

On the maker side the rejection is resolved before matching. A floored maker with any invalid oracle
has its risk-increasing orders pruned from the maker set, while its provably reducing orders stay
...
force-cancel path fails closed in the other direction. Being below the raw floor counts as grounds
only when every oracle is valid and the trusted value sits below it, so a bad price can never make
an account look breached to a keeper. The standalone lifecycle instructions apply the same rule. The
...
perp pnl leg. No stored twap is used to bound an invalid oracle, because the two sources can be
stale or wrong together, so a twap-derived allowance could understate a healthy account and falsely
arm the authority-wide breaker.
...
manually enrolled and monitored set, not an additional onchain state machine.
...
than either. It creates no new liability and no new exposure, and only deleverages. It stays allowed
...
of the input, and each leg is priced conservatively against the account. The input leg takes the
higher of the live price and the 5-minute TWAP, and the output leg takes the lower
(`InvalidSwap` otherwise), so a stale sample cannot flatter the exchange rate and the exemption
cannot be used to route value out of the account through a bad venue.
...
restricted to reduce-only activity but is not trippable, because the breaker needs equity strictly
...
subaccount's equity is below its raw floor. That is the raw floor and not the buffered line,
because the buffer gates actions while the floor arms the breaker. Positions with valid oracles are valued at live prices; invalid-oracle
liabilities and shorts receive their sound zero upper bound rather than blocking the proof, and
any invalid-oracle asset or long blocks it (see the oracle-validity section above), so an
...
1. **Lazily, on touch.** The actions that are allowed to run while a subaccount sits below its raw
   floor set the flag inline as a side effect of succeeding. Those actions are a reducing perp fill
   (whether the subaccount is taker or maker), a strictly reducing swap, and a trigger order
   cancelled by the below-floor check. A below-floor subaccount with resting reduce-side orders is therefore
...
2. **By the permissionless `tripEquityFloorBreaker` instruction.** Any keeper can call it against a
   ...
   instruction is permissionless, the guard bot is a backstop rather than a single point of
   failure. Any third party can trip a breached account.
...
authority-wide, so it freezes every subaccount under the authority, not just the one that breached.
...
credited side clears its buffered floor the exemption closes again. Partial cures compose, so
several subaccounts can each contribute what they have to spare. The SDK plans this.
...
requirement stays all-or-nothing and does not share the trip's liability/short upper bound. The
reset proves equity above the line, the direction in which conceding value to an unpriceable
position would be unsound, so a dead oracle on any position blocks the reset until the feed
recovers. When
...
- It does not close positions. A frozen account keeps its open positions, and passive losses can
  continue after the flag is set. Winding down is an operational step. Velocity's runbook uses the
...
Read the 70% floor configuration this way. No delegate action can take a subaccount
...
`getStatus` reports a level per subaccount (`disabled`, `healthy`, `warning`, `critical`,
`breached`) using the same thresholds Velocity's guard bot monitors. `disabled` covers a subaccount
with no floor set.
...
`0` while still holding buffer, and a check-disabled subaccount holds neither. (The rounding also means
...
That minimum would land the debited side exactly on its buffered floor. The SDK prices equity with
the same live-oracle rules as the program, but at the boundary the two can disagree by dust and
revert the transfer. `transferQuote` therefore evaluates the
...
Call it directly in these cases:

- rebalancing floor without moving funds, where a zero `amount` with a delta moves floor alone
  (this is what `rebalanceFloors` submits under the hood);
...
- a planned flow needs the delta without sending, in which case
  `manager.planQuoteTransfer(amount, from, to)` returns the padded parameters for inspection.
...
where equity backs it. Note what was conserved through every step, namely total floor (700,000 always),
...
Step 6: `rebalanceFloors` computes the same kind of move step 4 made by hand. It targets a
...
The manual move in step 4 over-rotated, leaving sub 1 with 73,500 of slack against sub 0's
206,500. The rebalancer evens the ratio out so both subaccounts are equally far from their
...
The SDK mirrors the onchain checks. All of them measure net equity through
`user.getFloorNetEquity()`, the same metric the program's gates use:

| Helper                                    | What it reports                                                                |
| ----------------------------------------- | ------------------------------------------------------------------------------ |
| `user.getFloorNetEquity()`                | Net equity, the value every floor check compares against                       |
| `user.isBelowEquityFloor(slot)`           | `true` when net equity is below the floor (point value)                        |
...
| `getEquityFloorLevel(equity, floor, buf)` | `disabled` / `healthy` / `warning` / `critical` / `breached`                   |
...
upper-bound verdict still proves. A keeper of its own should do the same. Note that the guard bot
feeds its level classifier `user.getNetUsdValue()` rather than `getFloorNetEquity()`, so its level
readout can differ from the manager's and the CLI's at the margin; the trip proof it sends is
unaffected.

## Using from Rust

`velocity-rs` carries the same helpers for Rust consumers. The generated bindings already include
...
let delta = calculate_equity_floor_auto_delta(
	amount,
	net_equity, // i128, from the program crate's calculate_net_equity_for_floor
	user.equity_floor,
...
guard bot, the CLI) calls the same classifier.
...
5. The delegate cures the breach while frozen, by depositing or by cure transfers from sibling
