...
- **`SpotMarket.if_last_settle_vault_amount: u64`** replaces the 8-byte trailing padding.
  ...
  the first revenue settle after upgrade; new markets initialize to 0), but custom decoders
  must add the field. It records the IF vault token balance as of the last revenue settle and
  is used as a donation-proof base for the per-period revenue-settle APR cap in
  `settle_revenue_to_insurance_fund` (`min(live_if_vault, snapshot)`), so a direct SPL
  donation into the IF vault right before a settle can no longer inflate the cap. No new
  instruction — the field is maintained automatically by the settle path.
