...
| revshare-reserve-net-user-pnl | Fix a High audit finding: the builder/referrer revenue-share sweep (`sweep_completed_revenue_share_for_market`, run on every permissionless `settle_pnl`/`settle_multiple_pnls` with a `RevenueShareEscrow` attached) paid rows out of a perp market's PnL pool after checking only the raw pool balance against `fees_accrued`, never reserving `max(net_user_pnl, 0)`. A caller could move pool tokens backing a third party's positive unsettled PnL, leaving that user's settlement short. The sweep now reserves the aggregate positive user claim — it draws only `pnl_pool_token_amount − max(net_user_pnl, 0)` — mirroring the protocol fee sweep (`sweep_market_fees`). `net_user_pnl` is valued at the market's oracle price, validity-gated in-slot by the settle that runs immediately before the sweep. No account-layout or IDL change (internal controller only); observable only as a settle side-effect that now stops sweeping revenue share once the PnL pool is down to reserved user claims |
