...
describe('velocity client', () => {
	...
	it('Initialize user account and deposit collateral', async () => {
		...
		const txSig = await velocityClient.depositIntoIsolatedPerpPosition(
			usdcAmount,
			0,
		...
		const depositTokenAmount =
			velocityClient.getIsolatedPerpPositionTokenAmount(0);
	...
	it('Transfer isolated perp position deposit', async () => {
		...
		const quoteTokenAmount = velocityClient.getQuoteAssetTokenAmount();
		...
		const quoteTokenAmoun2 = velocityClient.getQuoteAssetTokenAmount();
	...
	it('Long from 0 position', async () => {
		...
		console.log(
			...
			velocityClient.getIsolatedPerpPositionTokenAmount(0).toString()
		);
		assert(
			velocityClient.getIsolatedPerpPositionTokenAmount(0).eq(new BN(10000000))
		);
		assert(
			velocityClient
				.getUserStats()
	...
	it('Reverse long position', async () => {
		...
		assert.ok(
			velocityClient.getIsolatedPerpPositionTokenAmount(0).eq(new BN(9879998))
		);
		assert(
			velocityClient
				.getUserStats()
