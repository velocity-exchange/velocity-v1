...
pub struct OracleMap<'a> {
    ...
    pub slot: u64,
    /// Current slot duration in ms (from `State::slot_duration_ms()`), used to
...
impl<'a> OracleMap<'a> {
    ...
    pub fn get_account_info(&self, pubkey: &Pubkey) -> VelocityResult<AccountInfo<'a>> {
        Ok(self
            .oracles
            .get(pubkey)
    ...
    pub fn get_price_data(&mut self, id: &OracleIdentifier) -> VelocityResult<&OraclePriceData> {
    ...
    ) -> VelocityResult<(&OraclePriceData, OracleValidity)> {
    ...
    pub fn get_price_data_and_guard_rails(
        &mut self,
        oracle_id: &OracleIdentifier,
    ) -> VelocityResult<(&OraclePriceData, &ValidityGuardRails)> {
        if self.should_get_quote_asset_price_data(&oracle_id.0) {
    ...
    pub fn validate_oracle_account_info<'c>(account_info: &'c AccountInfo<'a>) -> VelocityResult {
