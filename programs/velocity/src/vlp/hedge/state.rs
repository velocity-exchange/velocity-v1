    ...
    crate::{
        ...
        math::{
            ...
            time::{
                ...
                LegacySlotDurationU64, Millis, SlotClock,
            },
        },
        state::{
...
impl LPPool {
    ...
    pub fn record_mint_redeem_fees(&mut self, amount: i64) -> VelocityResult {
    ...
    pub fn update_aum(
        ...
        for i in 0..self.constituents as usize {
            ...
            if !is_oracle_valid_for_action(
                ...
                Some(VelocityAction::UpdateLpPoolAum),
            )? {
                msg!(
...
impl Constituent {
    pub fn get_status(&self) -> VelocityResult<BitFlags<ConstituentStatus>> {
        BitFlags::<ConstituentStatus>::from_bits(usize::from(self.status)).safe_unwrap()
    }

    pub fn is_decommissioned(&self) -> VelocityResult<bool> {
        Ok(self
            .get_status()?
    ...
    /// account and in Velocity Borrow-Lend.
    pub fn get_full_token_amount(&self, spot_market: &SpotMarket) -> VelocityResult<i128> {
    ...
    pub fn record_swap_fees(&mut self, amount: i128) -> VelocityResult {
    ...
    pub fn get_notional(&self, price: i64, token_amount: i128) -> VelocityResult<i128> {
...
impl<'a> AccountZeroCopyMut<'a, TargetsDatum, ConstituentTargetBaseFixed> {
    pub fn update_target_base(
            ...
            .enumerate()
        {
            ...
            if slot_clock.elapsed(oldest_oracle_slot, slot) <= MAX_ORACLE_STALENESS_FOR_TARGET_CALC
            {
...
impl<'a> AccountZeroCopyMut<'a, AmmConstituentDatum, AmmConstituentMappingFixed> {
    #[cfg(test)]
    pub fn add_amm_constituent_datum(&mut self, datum: AmmConstituentDatum) -> VelocityResult<()> {
        let len = self.len();
    ...
    #[cfg(test)]
    pub fn sort(&mut self) -> VelocityResult<()> {
        let len = self.len();
...
impl ConstituentCorrelations {
    ...
    pub fn add_new_constituent(&mut self, new_constituent_correlations: &[i64]) -> VelocityResult {
    ...
    pub fn set_correlation(&mut self, i: u16, j: u16, corr: i64) -> VelocityResult {
...
impl<'a> AccountZeroCopy<'a, i64, ConstituentCorrelationsFixed> {
    pub fn get_correlation(&self, i: u16, j: u16) -> VelocityResult<i64> {
