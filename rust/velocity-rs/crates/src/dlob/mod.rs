...
pub struct DLOB {
    ...
    metadata: MetadataMap,
    /// static velocity program data e.g market tick sizes
    ...
    pub fn bids<'b>(
        ...
        let next_from = move || {
            ...
            if let Some(market) = perp_market {
                ...
                if let Some(x) = v {
                    if let Ok(liq) = program::vlp::amm::math::amm::calculate_amm_available_liquidity(
                        &market.amm,
