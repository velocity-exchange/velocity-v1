...
| #194 | Guard `add_insurance_fund_stake` against zero-share mints (cold-start first-staker inflation): a proportional round-down that would mint 0 shares now reverts with the new `IFStakeTooSmall` (6357) error, matching the existing `request_remove` guard. New error code only — no layout change (§5) |
