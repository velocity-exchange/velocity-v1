...
| --- | --- | --- | --- | --- |
| `PerpMarket.margin_ratio_initial` | `update_perp_market_margin_ratio` (`admin.rs:1439`) | warm | `MARGIN_PRECISION` (1e4) | 125..=10000 (`MIN`/`MAX_MARGIN_RATIO`, `constants.rs:203-204`); strictly > maintenance; `initial * 100 > amm.max_spread` (`validation/margin.rs:22-24,30-32,44-50`) |
...
| `PerpMarket.liquidator_fee` | `update_perp_liquidation_fee` (`admin.rs:1646`) | warm | `LIQUIDATION_FEE_PRECISION` (1e6) | fee sum (all three) strictly < 100% (`admin.rs:1659-1666`); cross-checked against maintenance margin (`validation/margin.rs:34-42`) |
| `PerpMarket.if_liquidation_fee` | same handler | warm | `LIQUIDATION_FEE_PRECISION` | < 100% standalone (`admin.rs:1668-1672`); in the sum and the margin cross-check |
| `PerpMarket.protocol_liquidation_fee` | same handler | warm | `LIQUIDATION_FEE_PRECISION` | <= 10% (`admin.rs:1674-1678`); in the sum; *not* in the margin cross-check |
| `SpotMarket.liquidator_fee` | `update_spot_market_liquidation_fee` (`admin.rs:1753`) | warm | `LIQUIDATION_FEE_PRECISION` | fee sum strictly < 100% (`admin.rs:1765-1772`) and nothing else; see [missing bounds](#missing-bounds-worth-knowing-about) |
| `SpotMarket.if_liquidation_fee` | same handler | warm | `LIQUIDATION_FEE_PRECISION` | <= 10% (`admin.rs:1774-1778`); in the sum |
| `SpotMarket.protocol_liquidation_fee` | same handler | warm | `LIQUIDATION_FEE_PRECISION` | <= 10% (`admin.rs:1780-1784`); in the sum |
| `PerpMarket.imf_factor` | `update_perp_market_imf_factor` (`admin.rs:2401`) | warm | `SPOT_IMF_PRECISION` (1e6); the field doc comment says `MARGIN_PRECISION` and is wrong (see below) | <= 1_000_000 (`admin.rs:2406-2410`) |
| `PerpMarket.unrealized_pnl_imf_factor` | same handler | warm | `SPOT_IMF_PRECISION`; same wrong doc comment | <= 1_000_000 (`admin.rs:2411-2415`) |
| `PerpMarket.unrealized_pnl_initial_asset_weight` | `update_perp_market_unrealized_asset_weight` (`admin.rs:2439`) | warm | `SPOT_WEIGHT_PRECISION` (1e4) | <= 10000 and <= the maintenance weight (`admin.rs:2444-2448,2454-2458`) |
| `PerpMarket.unrealized_pnl_maintenance_asset_weight` | same handler | warm | `SPOT_WEIGHT_PRECISION` | <= 10000 and >= the initial weight (`admin.rs:2449-2458`) |
| `PerpMarket.unrealized_pnl_max_imbalance` | `update_perp_market_max_imbalances` (`admin.rs:1544`) | warm | `QUOTE_PRECISION` | <= tier insurance cap + 1 (`admin.rs:1566-1574`; tier caps `constants.rs:147-150`) |
| `PerpMarket.insurance_claim.quote_max_insurance` | same handler | warm | `QUOTE_PRECISION` | <= tier insurance cap; >= insurance already settled (`admin.rs:1566-1581`) |
| `PerpMarket.insurance_claim.max_revenue_withdraw_per_period` | same handler | warm | `QUOTE_PRECISION` | <= max(tier cap, 250 USD); >= the current period's draw (`admin.rs:1566-1574,1606`) |
| `PerpMarket.contract_tier` | `update_perp_market_contract_tier` (`admin.rs:2380`) | warm | enum `ContractTier` (`state/perp_market.rs:80-94`) | **unchecked** |
| `State.initial_pct_to_liquidate` | `update_initial_pct_to_liquidate` (`admin.rs:2511`) | warm | `LIQUIDATION_PCT_PRECISION` (1e4, `constants.rs:46`) | **unchecked** (values above 10000 are inert: the sum is min'd at `math/liquidation.rs:440`) |
| `State.liquidation_duration` | `update_liquidation_duration` (`admin.rs:2525`) | warm | slots, u8 | **unchecked** (0 is safe by fallback: `math/liquidation.rs:437-438` defaults to 100% freeable) |
| `State.liquidation_margin_buffer_ratio` | `update_liquidation_margin_buffer_ratio` (`admin.rs:2539`) | warm | `MARGIN_PRECISION`; default 2% (`constants.rs:239`) | **unchecked** |
| `User.equity_floor` | `update_user_equity_floor` (`admin.rs:4385`) | warm | `QUOTE_PRECISION`; 0 disables | **unchecked** |
...
`PerpMarket::get_margin_ratio` (`state/perp_market.rs:808-838`). The initial ratio governs order
...
`min(liquidator_fee * 3, margin_ratio_maintenance * 100)` (`state/perp_market.rs:844-851`,
...
margin requirement types for large positions (`state/perp_market.rs:829`,
...
(`state/perp_market.rs:855-909`), with one exception: `unrealized_pnl_maintenance_asset_weight`
is returned untouched on the Maintenance branch (`state/perp_market.rs:909`), so lowering *that
...
(`state/perp_market.rs:874`), so the size of that discount moves with the oracle crank, not the
live price.
...
(`state/perp_market.rs:1015,1040,1195-1197`), and the insurance caps enforced by the *max
imbalances* setter (`admin.rs:1554-1564`). Two hazards: the change re-prioritizes liquidation
...
every subaccount to clear floor + buffer (`admin.rs:4364-4372`). The full lifecycle is in
[docs/EQUITY-FLOOR.md](./EQUITY-FLOOR.md).

**A doc-comment trap for operators:** the field docs of `PerpMarket.imf_factor` and
`unrealized_pnl_imf_factor` (`state/perp_market.rs:368-373`) claim `MARGIN_PRECISION` (1e4). The
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `SpotMarket.initial_asset_weight` | `update_spot_market_margin_weights` (`admin.rs:2007`) | warm | `SPOT_WEIGHT_PRECISION` (1e4) | market 0: exactly 10000; others: <= 10000 and <= maintenance weight; 0 allowed (`validation/margin.rs:64-69,92-106`) |
...
| `SpotMarket.optimal_utilization` | `update_spot_market_borrow_rate` (`admin.rs:2069`) | warm | `SPOT_UTILIZATION_PRECISION` (1e6) | <= 1_000_000 (`validation/spot_market.rs:17-22`) |
...
| `SpotMarket.min_borrow_rate` | same handler | warm | u8, 1 unit = 0.5% (`state/spot_market.rs:197-200`) | expanded (u8 x 5000) <= optimal rate; re-validated even when unchanged (`admin.rs:2084`, `validation/spot_market.rs:32-38`) |
| `SpotMarket.withdraw_guard_threshold` | `update_withdraw_guard_threshold` (`admin.rs:1813`) | warm | token mint precision | oracle live price and 5-min TWAP both > 0; notional at max(live, TWAP) <= $10,000 (`admin.rs:1833-1846`, `validation/spot_market.rs:46-74`, cap `constants.rs:255`) |
| `SpotMarket.withdraw_circuit_breaker_bps` | `update_spot_market_withdraw_circuit_breaker` (`admin.rs:2146`) | warm; **cold to exceed 25%** | bps (10000 = 100%) | <= 10000 for anyone; <= 2500 unless the signer is cold (`admin.rs:2150-2170`) |
| `SpotMarket.max_token_deposits` | `update_spot_market_max_token_deposits` (`admin.rs:2126`) | warm | token mint precision; 0 = no limit | **unchecked** |
| `SpotMarket.max_token_borrows_fraction` | `update_spot_market_max_token_borrows` (`admin.rs:2215`) | warm | fraction of `max_token_deposits`, x/10000 | derived cap must cover current outstanding borrows (`admin.rs:2228-2240`); the fraction itself may exceed 100% |
| `SpotMarket.deposit_guard_threshold` | `update_spot_market_deposit_cap` (`admin.rs:2188`) | warm | token mint precision | **unchecked** |
| `SpotMarket.max_deposit_bps_per_day` | same handler | warm | bps/day; 0 disables the limiter | **unchecked** (u16 caps it at 655%/day) |
| `SpotMarket.scale_initial_asset_weight_start` | `update_spot_market_scale_initial_asset_weight_start` (`admin.rs:2249`) | warm | `QUOTE_PRECISION` notional; 0 disables | **unchecked** |
| `SpotMarket.insurance_fund.unstaking_period` | `update_insurance_fund_unstaking_period` (`admin.rs:1733`) | warm | seconds (i64); default 13 days (`constants.rs:124`) | **unchecked** |
| `SpotMarket.insurance_fund.revenue_settle_period` | `update_spot_market_revenue_settle_period` (`admin.rs:1912`) | warm | seconds (i64) | > 0 only (`admin.rs:1919`) |
| `SpotMarket.insurance_fund.if_fee_factor` | `update_spot_market_if_factor` (`admin.rs:1863`) | warm | `IF_FACTOR_PRECISION` (1e6) | `if_fee_factor + protocol_fee_factor` strictly < 100% (`admin.rs:1885-1889`) |
| `SpotMarket.protocol_fee_factor` | same handler | warm | `IF_FACTOR_PRECISION` | same joint bound; no individual cap |
| `SpotMarket.if_paused_operations` | `update_spot_market_if_paused_operations` (`admin.rs:2289`) | pause | u8 bitmask of `InsuranceFundOperation` (`state/paused_operations.rs:88-94`) | pause admin may only add bits (`admin.rs:2296-2301`, `auth.rs:112-122`); warm/cold set anything |
...
`initial_asset_weight > 0` (`admin.rs:1979-2002`), but this weights handler can raise the weight
...
(`constants.rs:261-268`), `max_borrow_rate` is the rate at 100% utilization, and the expanded
...
cap (`admin.rs:4673-4684`). `withdraw_circuit_breaker_bps` sets how much of the 24h deposit TWAP
...
freeze the whole accrual block (the comment at `admin.rs:1879-1884` spells this out); the same
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `SpotMarket.oracle` + `oracle_source` | `update_spot_market_oracle` (`admin.rs:1011`) | **cold** | Pubkey + `OracleSource` enum | account must decode under the source; new price > 0 and within 10% of the old (`admin.rs:1021-1091`), unless `skip_invariant_check=true`; four Pyth-pull variants rejected (`admin.rs:100-112`); see caveats below |
| `PerpMarket.oracle` + `oracle_source` | `update_perp_market_oracle` (`admin.rs:2615`) | **cold** | same | same checks (`admin.rs:2627-2698`), same caveats |
| `State.oracle_guard_rails.*` (all 6 fields) | `update_oracle_guard_rails` (`admin.rs:2556`) | warm | see below | **unchecked**: one bare struct assignment (`admin.rs:2566`), no validator anywhere |
| `PerpMarket.oracle_low_risk_slot_delay_override` | `update_perp_market_oracle_low_risk_slot_delay_override` (`admin.rs:2986`) | warm | slots (i8); 0 = fall back to state | **unchecked** |
| `PerpMarket.oracle_slot_delay_override` | `update_perp_market_oracle_slot_delay_override` (`admin.rs:3006`) | warm | slots (i8); negative = unset; 0 disables immediate AMM fills | **unchecked** |
| `PrelaunchOracle.price` / `max_price` | `update_prelaunch_oracle_params` (`admin.rs:3200`) | warm | `PRICE_PRECISION` (1e6) | both != 0 and `price <= max_price` (`state/oracle.rs:758-772`); no sign or magnitude bound |
| `State.feature_bit_flags` MmOracleUpdate bit | `update_feature_bit_flags_mm_oracle` (`admin.rs:4071`) | hot (`HotRole::FeatureFlag`) to disable; **cold to re-enable** (`admin.rs:4077-4081`) | one bit | single-bit read-modify-write; other bits undisturbed |
...
- `price_divergence.mark_oracle_percent_divergence` (`PERCENTAGE_PRECISION`, default 10%).
  ...
  (`controller/orders.rs:1556-1610` via `validate_market_within_price_band`) and the funding
...
- `validity.confidence_interval_max_size` (`BID_ASK_SPREAD_PRECISION`, default 2%). Multiplied
  by the market's tier-keyed 1x-50x confidence multiplier (`state/perp_market.rs:633-643`,
...
unset (resolving to a 2-slot gap for MM-sourced prices, `constants.rs:271`); a high positive
value lets JIT fills execute against a price up to 127 slots old (plus up to 2 slots of hidden
MM-oracle source age, `constants.rs:282-289`), exposing counterparties to stale-price
arbitrage. Note the accounts struct is named `HotAdminUpdatePerpMarket` but its constraint is
`check_warm` (`admin.rs:4571`); the real minimum tier is warm.

The field doc comment on `oracle_low_risk_slot_delay_override`
(`state/perp_market.rs:439-441`) still describes an auction speed-bump override; the code uses
...
overwrites the market's mark TWAPs (`admin.rs:3215-3225`), moving every open position's
...
prices (`admin.rs:3560-3563,3852-3855`). Positions do not freeze; pricing falls back to the
...
governed by hard-coded constants, not admin parameters (`constants.rs:271-289`).
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `State.exchange_status` | `update_exchange_status` (`admin.rs:3089`) | pause | u8 bitmask, 8 `ExchangeStatus` bits (`state/state.rs:117-129`) | pause admin add-only (`admin.rs:3095`); warm/cold set anything; all 8 bits defined |
| `PerpMarket.status` | `update_perp_market_status` (`admin.rs:2312`) | warm | `MarketStatus` enum | Settlement and Delisted rejected (`admin.rs:2316-2320`); no transition checks within {Initialized, Active, ReduceOnly} |
| `SpotMarket.status` | `update_spot_market_status` (`admin.rs:1932`) | warm | `MarketStatus` enum | **unchecked**: any of the 5 variants, Delisted included; see below |
| `PerpMarket.paused_operations` | `update_perp_market_paused_operations` (`admin.rs:2339`) | pause | u8 bitmask, 8 `PerpOperation` bits (`state/paused_operations.rs:6-16`) | three-way: cold anything; warm only the `UpdateFunding`/`SettleRevPool` bits (`admin.rs:2351-2361`); pause admin add-only (`admin.rs:2362-2367`) |
| `SpotMarket.paused_operations` | `update_spot_market_paused_operations` (`admin.rs:1952`) | pause | u8 bitmask, 5 `SpotOperation` bits (`state/paused_operations.rs:57-64`) | pause admin add-only (`admin.rs:1961-1966`); warm/cold anything; undefined bits 5-7 stored but inert |
...
| `SpotMarket.expiry_ts` + status | `update_spot_market_expiry` (`admin.rs:1101`) | warm | Unix seconds | expiry must be in the future (`admin.rs:1109-1113`); status hard-coded to ReduceOnly (`admin.rs:1127`) |
| `PerpMarket.expiry_ts` + status | `update_perp_market_expiry` (`admin.rs:1136`) | warm | Unix seconds | expiry must be in the future (`admin.rs:1144-1148`); status hard-coded to ReduceOnly (`admin.rs:1162`) |
| `SpotMarket.orders_enabled` | `update_spot_market_orders_enabled` (`admin.rs:2269`) | warm | bool | **unchecked**, and dead: no program code reads it (see below) |
| `State.solvency_status` | `update_solvency_status` (`admin.rs:3105`) | **cold** | u8, only bit 0 defined (`state/state.rs:140-145`) | **unchecked**: any u8 stored; values >= 2 brick the readers (see below) |
| `UserStats.paused_operations` | `admin_update_user_stats_paused_operations` (`admin.rs:3147`) | pause (or hot `UserFlag`) | u8 bitmask, 3 bits (`state/user.rs:2143-2150`) | pause admin add-only (`admin.rs:3161-3167`); cold/warm/hot-UserFlag set anything |
| `State.feature_bit_flags`: MedianTriggerPrice, BuilderCodes | `update_feature_bit_flags_*` (`admin.rs:4092,4113`) | hot (`FeatureFlag`) to disable; **cold to enable** | one bit each (`state/state.rs:398-404`) | single-bit read-modify-write; asymmetric authority in-handler (`admin.rs:4098-4102,4119-4123`) |
| `State.lp_pool_feature_bit_flags`: SettleLpPool, SwapLpPool, MintRedeemLpPool | `update_feature_bit_flags_*_lp_pool` (`admin.rs:4155,4176,4197`) | hot to disable; cold to enable | one bit each (`state/state.rs:406-411`) | same asymmetry; consumers compiled out of mainnet builds (`vlp-hedge` feature) |
| `PerpMarket.hedge_config.paused_operations` | `update_perp_market_lp_pool_paused_operations` (`admin.rs:2964`) | pause | u8 bitmask, 2 `PerpLpOperation` bits (`state/paused_operations.rs:117-121`) | pause admin add-only (`admin.rs:2972-2977`); no Delisted gate |
...
(`controller/orders.rs:175,1106,1674`); positions can still be closed. Initialized is much
...
the pause admin (`admin.rs:2354`). A single key holding both the warm and pause roles therefore
...
(`state/state.rs:238-240`, `math/safe_unwrap.rs:32-47`). Other bitmask setters guard against
unknown bits (`admin.rs:2225-2232,4260-4267`); this one does not.
...
moving the bid/ask TWAP (`state/user.rs:2120-2126`).
...
which reference price trigger orders evaluate against (`controller/orders.rs:3778`,
`state/perp_market.rs:1089-1108`), so the same oracle print can trigger a stop under one setting
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `PerpMarket.order_step_size` | `update_perp_market_step_size_and_tick_size` (`admin.rs:2717`) | warm | `BASE_PRECISION` (1e9) | 1..=2_000_000_000 (`admin.rs:2725-2726`); no divisibility re-check against existing state (see below) |
| `PerpMarket.order_tick_size` | same handler | warm | `PRICE_PRECISION` (1e6) | > 0 only (`admin.rs:2725`); no upper bound |
| `PerpMarket.market_stats.min_order_size` | `update_perp_market_min_order_size` (`admin.rs:2748`) | warm | `BASE_PRECISION` | > 0 only (`admin.rs:2755`); no upper bound, no tie to step size |
| `PerpMarket.max_open_interest` | `update_perp_market_max_open_interest` (`admin.rs:2828`) | warm | `BASE_PRECISION` (u128); 0 = uncapped | must be a multiple of the current step size and fit u64 (`admin.rs:2835-2842`) |
| `State.min_perp_auction_duration` | `update_perp_auction_duration` (`admin.rs:3119`) | warm | slots (u8) | **unchecked** (0-255) |
| `State.default_spot_auction_duration` | `update_spot_auction_duration` (`admin.rs:3133`) | warm | slots (u8) | **unchecked**, and no on-chain reader exists |
| `SpotMarket.order_step_size` / `order_tick_size` | `update_spot_market_step_size_and_tick_size` (`admin.rs:2770`) | warm | token precision / `PRICE_PRECISION` | > 0 required, except **market 0 is fully exempt** (`admin.rs:2778-2781`); effectively inert (spot DLOB disabled) |
| `SpotMarket.min_order_size` | `update_spot_market_min_order_size` (`admin.rs:2803`) | warm | token precision | > 0 except market 0 exempt (`admin.rs:2810-2813`); no on-chain reader |
| `SpotMarket.pool_id` | `update_spot_market_pool_id` (`admin.rs:461`) | warm | u8 pool tag (0 = main, 2 = LST, `constants.rs:258`) | value **unchecked**; market status must be Initialized (`admin.rs:472-476`) |
| `PerpMarket.hedge_config.pool_id` | `update_perp_market_lp_pool_id` (`admin.rs:1714`) | warm | u8 LP-pool id | **unchecked**; no cross-check against any LPPool account |
...
(`state/perp_market.rs:988-995`: OI/1000 clamped into [100x, 5000x] of it) and AMM fallback
...
(`controller/orders.rs:1512-1525`, strict >). Setting it to a nonzero value at or below current
...
(`controller/orders.rs:3798-3804`, relied on by the TWAP-manipulation defense at
`constants.rs:213-227`). At 255 slots (~102s) every market order's auction stretches
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `State.perp_fee_structure` (10 fee tiers, filler rewards, AMM/IF split) | `update_perp_fee_structure` (`admin.rs:2479`) | warm | tier fees in tenths of a bp (numerator over denominator >= `FEE_DENOMINATOR` 100_000, `constants.rs:159`); filler/referral in percent; `flat_filler_fee` in `QUOTE_PRECISION` | taker fee <= 30 bps; maker rebate <= 3 bps; referee/referrer <= 100%; filler reward <= 20%; `flat_filler_fee` <= $0.01; `amm + if` numerators <= 100; two derived consistency checks (fill must leave the market at least the maker rebate) (`validation/fee_structure.rs:15-137`). No lower bounds: an all-zero schedule is legal |
| `State.spot_fee_structure` | `update_spot_fee_structure` (`admin.rs:2495`) | warm | same | same validator; almost entirely inert (see below) |
| `PerpMarket.fee_adjustment` | `update_perp_market_fee_adjustment` (`admin.rs:2857`) | warm | signed percent of the base fee, i16 | -100..=100 (`admin.rs:2864-2870`, `FEE_ADJUSTMENT_MAX` `constants.rs:177`) |
| `SpotMarket.fee_adjustment` | `update_spot_market_fee_adjustment` (`admin.rs:3026`) | warm | same | same bound; **no reader exists** (see below) |
| `PerpMarket.fee_pool_buffer_target` | `update_perp_market_fee_pool_buffer_target` (`admin.rs:2882`) | warm | `QUOTE_PRECISION` | **unchecked** |
| `PerpMarket.bankruptcy_if_floor_pct` | `update_perp_market_bankruptcy_if_floor_pct` (`admin.rs:2902`) | warm | `PERCENTAGE_PRECISION` (u32); 0 disables | <= 1_000_000 = 100% of OI notional (`admin.rs:2909-2913`) |
| `State.feature_bit_flags` VammMakerRebate bit | `update_feature_bit_flags_vamm_maker_rebate` (`admin.rs:4134`) | hot (`FeatureFlag`) to disable; **cold to enable** (`admin.rs:4140-4144`) | one bit | single-bit read-modify-write |
| `State.protocol_fee_recipient_perp` / `_spot` | `update_protocol_fee_recipient` (`admin.rs:4885`) | **cold** | Pubkey | **unchecked** (any 32 bytes, `Pubkey::default()` included) |

### What a change does

**Perp fee structure.** Rewrites the global schedule read on every perp fill
(`controller/orders.rs:1422` into `math/fees.rs:78,353`): taker fee, maker rebate, filler
...
force-cancelled orders (`controller/orders.rs:1325,1378,1468,3927,4123`).

**Per-market fee adjustment** scales the taker fee and maker rebate by up to +/-100% on every
fill in that market (`math/fees.rs:217-276`) and shifts the post-only limit-price buffer
(`math/orders.rs:129-160`), all read live.
...
`open_interest x oracle TWAP x pct` (`state/perp_market.rs:925-969`), reserved at
...
(`controller/orders.rs:4097`). The operator hazard is a false sense of control: changing spot
...
| Field | Set by | Tier | Units | Enforced bounds |
| --- | --- | --- | --- | --- |
| `PerpMarket.market_stats.funding_period` | `update_perp_market_funding_period` (`admin.rs:1478`) | warm | seconds (i64) | >= 0 only (`admin.rs:1489`); downstream math imposes a hidden functional ceiling (see below) |
| `PerpMarket.funding_clamp_threshold` | `update_perp_market_funding_dead_zone` (`admin.rs:1504`) | warm | bps (`BPS_PRECISION` 10000) | < 10000, strict (`admin.rs:1517-1520`) |
| `PerpMarket.funding_ramp_slope` | same handler | warm | multiplier at `PERCENTAGE_PRECISION`; default 1.0x | > 0 only (`admin.rs:1522`); no upper cap (u32, up to ~4294x) |
| `PerpMarket.pnl_pool` (credit) | `update_perp_market_pnl_pool` (`admin.rs:1319`) | warm | quote token amount | no numeric cap, but the credit must leave the vault covering all depositor claims (`admin.rs:1335`, `math/spot_withdraw.rs:532-547`); no tokens move |
| `PerpMarket.name` | `update_perp_market_name` (`admin.rs:1614`) | warm | [u8; 32] | **unchecked** |
| `SpotMarket.name` | `update_spot_market_name` (`admin.rs:1627`) | warm | [u8; 32] | reserved name "USDT" only on market 0 (`admin.rs:1632-1637`); otherwise anything |
| `SpotMarket.asset_tier` | `update_spot_market_asset_tier` (`admin.rs:1979`) | warm | `AssetTier` enum (5 variants, `state/spot_market.rs:750-762`) | must be Collateral/Protected while `initial_asset_weight > 0` (`admin.rs:1986-1992`); otherwise any variant |
| `State.settlement_duration` | `update_state_settlement_duration` (`admin.rs:2570`) | warm | seconds (u16) | **unchecked** (0 to ~18.2h) |
| `State.max_number_of_sub_accounts` | `update_state_max_number_of_sub_accounts` (`admin.rs:2584`) | warm | count (u16); values > 5 mean value x 100 (`state/state.rs:248-254`); 0 = unlimited | **unchecked** |
| `State.max_initialize_user_fee` | `update_state_max_initialize_user_fee` (`admin.rs:2598`) | warm | hundredths of 1 SOL (u16) | **unchecked** (up to 655.35 SOL) |
| `PerpMarket.number_of_users` / `number_of_users_with_base` | `update_perp_market_number_of_users` (`admin.rs:2925`) | warm | counts (u32) | only `users >= users_with_base` (`admin.rs:2955-2959`); no check against the true count; works on any market status |
| `PerpMarket.market_config` (DisableFormulaicKUpdate bit) | `update_perp_market_config` (`admin.rs:4221`) | warm to clear; **cold-key to set** (`admin.rs:4237-4243`) | u8, 1 defined bit | unknown bits rejected (`admin.rs:4227-4232`) |
| `User.special_user_status` (VammHedger bit) | `update_special_user_status` (`admin.rs:4256`) | hot (`UserFlag`) to clear; **cold-key to set** (`admin.rs:4271-4277`) | u8, 1 defined bit | unknown bits rejected (`admin.rs:4262-4267`) |
| `State.discount_mint` | `update_discount_mint` (`admin.rs:3075`) | warm | Pubkey | **unchecked**, and no reader exists (vestigial) |
| `State.whitelist_mint` | `update_whitelist_mint` (`admin.rs:3061`) | none: **not callable** | Pubkey | the lib.rs wrapper is commented out (`lib.rs:1680-1685`); the field is frozen at `Pubkey::default()` (whitelist off) |
| `State.cold_admin` | `update_admin` (`admin.rs:3051`) | **cold** | Pubkey | **unchecked** (`Pubkey::default()` accepted) |
| `State.warm_admin` | `update_warm_admin` (`admin.rs:4846`) | **cold** | Pubkey | **unchecked** |
| `State.pause_admin` | `update_pause_admin` (`admin.rs:4856`) | **cold** | Pubkey | **unchecked** |
| `State.hot_<role>` (12 roles) | `update_hot_admin` (`admin.rs:4870`) | warm | Pubkey per `HotRole` (`state/state.rs:102-115`) | **unchecked** |
...
`controller/orders.rs:1540-1549`), and the downstream math has two revert bands: above 86,400s
...
the mark-TWAP window to the last print (`math/stats.rs:77`), defeating the mark/oracle
divergence rails, and divides by zero in the median-trigger-price basis when that feature is on
(`state/perp_market.rs:1169-1182`). A larger period also *raises* the per-interval funding rate
...
(`state/state.rs:256-275`), and any nonzero value also gates deletion of fresh, non-idle
...
(`admin.rs:909-914`) and the final delist sweep (`admin.rs:1211-1220`). Overriding
...
(`admin.rs:171-177`). Perp names carry no such rule, so a perp rename can still mislead
dashboards.
...
1. **`funding_period` can halt all fills in a market** (`admin.rs:1489` checks only `>= 0`).
   ...
   divergence rails (`math/stats.rs:77`).
2. **Spot `MarketStatus::Delisted` is an irreversible withdrawal freeze.** The status setter
   accepts any variant unchecked (`admin.rs:1932-1947`), withdrawals reject Delisted
...
3. **Spot `liquidator_fee` has no relationship to the liability weights**
   (`admin.rs:1765-1772` bounds only the three-fee sum below 100%). A fee above the market's
...
4. **The six oracle guard rails are one unchecked struct write** (`admin.rs:2566`). Sharpest
...
5. **`update_solvency_status` accepts bytes its own readers cannot decode.** Only bit 0 is
   ...
   `FailedUnwrap` until rewritten (`state/state.rs:238-240`). The unknown-bit guard used by
   `market_config` and `special_user_status` (`admin.rs:4227-4232,4262-4267`) is missing here.
6. **`contract_tier` is unchecked and desynchronizes the insurance caps** (`admin.rs:2380`).
...
7. **Perp `order_step_size` is not re-checked against existing state** (`admin.rs:2717`). A
...
9. **`insurance_fund.unstaking_period` is a fully unchecked signed value** (`admin.rs:1733`),
...
10. **`liquidation_margin_buffer_ratio` is an uncapped u32** (`admin.rs:2539`). Large values
...
11. **`scale_initial_asset_weight_start` treats 1 and 0 very differently** (`admin.rs:2249`,
...
12. **`deposit_guard_threshold` lacks the $10k notional cap its withdraw-side mirror carries**
    (`admin.rs:2188` vs `validation/spot_market.rs:46-74`), so the deposit rate limiter can be
    disabled by one warm write.
13. **`fee_pool_buffer_target` is unchecked and silently stalls sweeps** (`admin.rs:2882`): an
...
14. **`SpotMarket.pool_id` can be repointed on a live market** (`admin.rs:461`): the
...
15. **`cold_admin` accepts `Pubkey::default()`** (`admin.rs:3051`). That write permanently
...
- **Key collisions defeat the tier matrix.** A key that is both warm and pause admin bypasses
  the add-only rule and the perp pause bit budget (`auth.rs:118`, `admin.rs:2354`). The
...
- **Doc-comment precision bugs invite 100x mis-sets.** `PerpMarket.imf_factor`,
  `PerpMarket.unrealized_pnl_imf_factor` (`state/perp_market.rs:368-373`) and
  ...
  `oracle_low_risk_slot_delay_override` doc comment (`state/perp_market.rs:439-441`) describes
