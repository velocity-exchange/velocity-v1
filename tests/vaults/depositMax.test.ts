...
describe('velocityVaults', () => {
	...
	it('user deposit beyond max', async () => {
		...
		expect(Number(user1TokenBalance0.amount)).to.deep.equal(
			usdcAmount.toNumber()
		);
		...
		expect(vault0.totalShares.toNumber()).to.deep.equal(0);
		expect(vault1.totalShares.toNumber()).to.deep.equal(
			vault1.maxTokens.toNumber()
		);
		...
		expect(Number(user1TokenBalance1.amount)).to.deep.equal(
