...
use program::state::perp_market::MarketStats;
...
fn dlob_find_crosses_for_auctions_vamm_reduce_only_min_size() {
    ...
    market_bid_small_reduce_only.reduce_only = true;
    market_bid_small_reduce_only.auction_duration = 10;
    market_bid_small_reduce_only.auction_start_price = taker_price_i64;
    market_bid_small_reduce_only.auction_end_price = taker_price_i64;
    dlob.insert_order(&Pubkey::new_unique(), slot, market_bid_small_reduce_only);
    ...
    // Should find 3 crosses (all cross limit orders)
    assert_eq!(crosses.crosses.len(), 3);
    ...
    let cross_small_reduce_only = crosses
        .crosses
        .iter()
...
fn market_order_get_price_same_start_end_price() {
    ...
    for direction in [Direction::Long, Direction::Short] {
        ...
        let price_at_start =
            order.get_price(start_slot, oracle_price, tick_size, SlotClock::baseline());
        assert_eq!(
            price_at_start,
...
fn dlob_oracle_order_negative_offset_prices_post_auction() {
    ...
    let price = order.get_price(200, oracle_price, tick_size, SlotClock::baseline());
    ...
    assert_eq!(
        no_offset.get_price(200, oracle_price, tick_size, SlotClock::baseline()),
        None
    );
    ...
    assert_eq!(
        pos_offset.get_price(200, oracle_price, tick_size, SlotClock::baseline()),
        Some(100_250)
    );
}
...
/// Resting orders use the same min-size distinction as placement validation:
...
fn post_trigger_price_mirrors_program_trigger_auction_params() {
    ...
    let fast_clock = SlotClock::from_state_fields([1, 1, 1, 1], 0, 0, 0);
