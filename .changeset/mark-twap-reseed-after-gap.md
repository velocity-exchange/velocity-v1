...
the opposing weight at 1, so past one funding period a single sample replaces the TWAP outright. The
program now discards the stored mark TWAPs and re-seeds them from the oracle TWAP when they went
unwritten for more than `max(fundingPeriod * 3, 3600)` seconds. The first funding update after such a
gap therefore sees a zero price spread and charges the baseline offset alone, and the real premium
