...
| #243      | Spot liquidations price seized collateral protectively when the deposit oracle is margin-invalid (`StaleForMargin`/`TooUncertain` but still `Liquidate`-acceptable): `liquidate_spot`, `liquidate_spot_with_swap_begin`/`_end`, and `liquidate_perp_pnl_for_deposit` size the asset transfer (and the swap worst-case price) at `max(oracle, 5min twap, oracle + confidence)` instead of the raw oracle price. Liquidatability itself is unchanged (a stale price can still flag the account), but a depressed/uncertain price can no longer cheapen the collateral a liquidator receives — liquidators may receive less asset per liability repaid than the raw oracle implies. SDK adds `calculateUserProtectiveAssetPrice`; pass it as `assetPrice` to `calculateAssetTransferForLiabilityTransfer` when the deposit oracle is margin-invalid. No layout/IDL change |
