...
fn idl_type_to_rust(t: &IdlType) -> String {
    match t {
        ...
        // anchor's `IdlType` is `#[non_exhaustive]`. Unreachable for current velocity IDL.
...
fn generate_idl_types(idl: &Idl) -> String {
        ...
        let type_tokens = match &type_def.ty {
            IdlTypeDefTy::Enum { variants } => {
                ...
                if has_complex_first {
                    // TODO: complex-first enums get no `Default`. Not currently used by velocity.
                    quote! {
    ...
    let output = quote! {
        ...
        //!
        //! Auto-generated IDL types, manual edits do not persist (see `crates/velocity-idl-gen`)
        //!
