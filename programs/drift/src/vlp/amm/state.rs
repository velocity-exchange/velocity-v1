...
///   `handle_update_amm_<field>` so the audit `rg 'market\.amm\.\w+\s*=' programs/drift/src/`
...
impl AMM {
    ...
    pub fn get_lower_bound_sqrt_k(self, min_order_size: u64) -> DriftResult<u128> {
        Ok(self.sqrt_k.min(
    ...
    pub fn can_lower_k(&self, min_order_size: u64) -> DriftResult<bool> {
    ...
    pub fn update_concentration_coef(&mut self, scale: u128) -> DriftResult {
    ...
    pub fn recenter(&mut self, peg_multiplier: u128, sqrt_k: u128) -> DriftResult {
        use crate::math::bn;
