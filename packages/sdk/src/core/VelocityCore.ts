...
export class VelocityCore {
	 ...
	 * @param args.clobAccounts - the market's CLOB accounts (`quoter` registry entry, writable `clobMarket`, `clobProgram`, `quoterSigner`, optionally the writable `crankConditions` wake-hint account); when passed, an unfilled limit remainder rests on the CLOB instead of being cancelled.
