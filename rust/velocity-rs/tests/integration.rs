...
async fn client_subscribe_swift_orders() {
    ...
    let mut order_stream = client
        .subscribe_swift_orders(all_markets.as_slice(), Some(false), None)
        .await
        .unwrap();
...
async fn place_order_sim_via_privy_account() {
    ...
    let taker_order_params = OrderParams {
        order_type: velocity_rs::types::OrderType::Oracle,
        market_type: velocity_rs::types::MarketType::Perp,
        direction: velocity_rs::types::PositionDirection::Long,
