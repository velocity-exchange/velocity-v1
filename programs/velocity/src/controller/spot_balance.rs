...
/// depressed oracle normalize away the very check meant to stop it. Liquidation legitimately
/// advances these TWAPs and does gate on them, so the refresh stays and moves after the gate.
pub fn update_spot_market_and_check_validity(
    ...
    let validity = if spot_market.market_index == QUOTE_SPOT_MARKET_INDEX {
