...
| oracle-F4 #14 | PnL-vs-spot liquidation routes (`liquidate_perp_pnl_for_deposit`, `liquidate_borrow_for_perp_pnl`) skip the oracle-vs-5min-TWAP band that direct spot liquidation enforces. Rather than block these liquidations (PR #243 philosophy), when the deposit/borrow oracle is margin-invalid (`StaleForMargin`/`TooUncertain`) the collateral↔pnl conversion is now priced at a user-protective bound — asset leg `max(oracle, 5min twap, oracle+conf)`, liability leg `min(oracle, 5min twap, oracle−conf)` floored at 1 — instead of the raw oracle price. Valuation (margin shortage → tokens) still uses the raw oracle price. New SDK exports `calculateUserProtectiveAssetPrice`/`calculateUserProtectiveLiabilityPrice` (§4.6). No layout/IDL change |
