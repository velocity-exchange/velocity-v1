...
impl InsuranceFundStake {
    ...
    fn validate_base(&self, spot_market: &SpotMarket) -> VelocityResult {
        validate!(
    ...
    pub fn checked_if_shares(&self, spot_market: &SpotMarket) -> VelocityResult<u128> {
    ...
    pub fn increase_if_shares(&mut self, delta: u128, spot_market: &SpotMarket) -> VelocityResult {
        self.validate_base(spot_market)?;
        safe_increment!(self.if_shares, delta);
        Ok(())
    }

    pub fn decrease_if_shares(&mut self, delta: u128, spot_market: &SpotMarket) -> VelocityResult {
        self.validate_base(spot_market)?;
