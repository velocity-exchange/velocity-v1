...
use velocity_rs::{
    ...
    // The auction subscriber yields the SDK's IDL `User`; we only need the
    // account key from it and re-fetch the program `User` (above) so the types
    // line up with `JitProxyClient`.
    velocity_idl::accounts::User as AuctionUser,
...
impl Jitter {
    ...
    pub fn new_with_shotgun(
        ...
        let jit_proxy_client = JitProxyClient::new(velocity_client.clone(), config, cu_params);
        let shotgun = Arc::new(Shotgun {
            jit_proxy_client,
            authority: *velocity_client.wallet().authority(),
        });
    ...
    pub async fn subscribe(self: Arc<Self>, url: String) -> JitResult<AuctionSubscriber> {
        ...
        let markets = self.velocity_client.get_all_perp_market_ids();
        let mut swift_order_stream = self
            .velocity_client
            .subscribe_swift_orders(&markets, Some(true), None, None)
            .await
            .map_err(|err| {
    ...
    pub async fn on_auction(&self, auction: ProgramAccountUpdate<AuctionUser>) -> JitResult<()> {
        log::info!("Auction received");
        let user_pubkey = &auction.pubkey;
        // Re-fetch the program `User` so all downstream types (orders,
        // user_stats, jit ix) share the program account layout that
        // `JitProxyClient` expects.
        let taker_key = Pubkey::from_str(user_pubkey).unwrap();
        let user: User = self.velocity_client.get_user_account(&taker_key).await?;
            ...
            match order.market_type {
                MarketType::Perp => {
                    if let Some(param) = self.perp_params.get(&order.market_index) {
                        ...
                        let min_order_size = perp_market.market_stats.min_order_size;
                ...
                MarketType::Spot => {
                    if let Some(param) = self.spot_params.get(&order.market_index) {
                        ...
                        let taker_stats: UserStats =
                            self.velocity_client.get_user_stats(&user.authority).await?;
                        let referrer_info = ReferrerInfo::get_referrer_info(taker_stats);
    ...
    pub async fn on_swift_order(&self, signed_order_info: SignedOrderInfo) -> JitResult<()> {
        ...
        let taker_pubkey = signed_order_info.taker_subaccount();
        ...
        let (taker, taker_stats) = tokio::try_join!(
            self.velocity_client.get_user_account(&taker_pubkey),
            self.velocity_client.get_user_stats(taker_authority),
        )?;
