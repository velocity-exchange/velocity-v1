...
pub fn is_multiple_of_step_size(base_asset_amount: u64, step_size: u64) -> VelocityResult<bool> {
    let remainder = base_asset_amount
        .checked_rem_euclid(step_size)
        .ok_or_else(math_error!())?;
...
pub fn order_satisfies_trigger_condition(order: &Order, oracle_price: u64) -> VelocityResult<bool> {
...
) -> VelocityResult<(Option<u64>, Option<u64>)> {
    let direction: PositionDirection = user.orders[user_order_index].direction;
    match direction {
        PositionDirection::Long => {
            let max_quote =
                get_max_fill_amounts_for_market(user, quote_market, is_leaving_velocity)?
                    .cast::<u64>()?;
            Ok((None, Some(max_quote)))
        }
        PositionDirection::Short => {
            let max_base = standardize_base_asset_amount(
                get_max_fill_amounts_for_market(user, base_market, is_leaving_velocity)?
                    .cast::<u64>()?,
...
pub fn estimate_price_from_side(side: &Vec<Level>, depth: u64) -> VelocityResult<Option<u64>> {
...
pub fn select_margin_type_for_perp_maker(
    ...
    market_index: u16,
) -> VelocityResult<(MarginRequirementType, bool)> {
