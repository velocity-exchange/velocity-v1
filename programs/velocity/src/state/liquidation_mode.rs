...
pub trait LiquidatePerpMode {
    fn user_is_being_liquidated(&self, user: &User) -> VelocityResult<bool>;
    ...
    fn enter_liquidation(&self, user: &mut User, slot: u64) -> VelocityResult<u16>;

    fn can_exit_liquidation(&self, margin_calculation: &MarginCalculation) -> VelocityResult<bool>;

    fn exit_liquidation(&self, user: &mut User) -> VelocityResult<()>;
    ...
    fn exit_bankruptcy(&self, user: &mut User) -> VelocityResult<()>;

    fn get_event_fields(
        &self,
        margin_calculation: &MarginCalculation,
    ) -> VelocityResult<(u128, i128, u8)>;

    fn validate_spot_position(&self, user: &User, asset_market_index: u16) -> VelocityResult<()>;

    fn get_spot_token_amount(&self, user: &User, spot_market: &SpotMarket) -> VelocityResult<u128>;
    ...
    fn margin_shortage(&self, margin_calculation: &MarginCalculation) -> VelocityResult<u128>;
}

pub fn get_perp_liquidation_mode(
    user: &User,
    market_index: u16,
) -> VelocityResult<Box<dyn LiquidatePerpMode>> {
...
impl LiquidatePerpMode for IsolatedMarginLiquidatePerpMode {
    ...
    fn increment_free_margin(&self, _user: &mut User, _amount: u64) -> VelocityResult<()> {
        Ok(())
    }
    ...
    fn validate_spot_position(&self, _user: &User, asset_market_index: u16) -> VelocityResult<()> {
        validate!(
