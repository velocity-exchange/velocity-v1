...
impl HistoricalIndexData {
    ...
    pub fn default_with_current_oracle(oracle_price_data: OraclePriceData) -> VelocityResult<Self> {
    ...
    pub fn new(
        ...
        let (safe_oracle_price_data, safe_price_is_mm_sourced) = if exchange_oracle_is_more_recent
            ...
            || !is_oracle_valid_for_action(
                mm_oracle_validity,
                Some(VelocityAction::UseMMOraclePrice),
            )?
...
impl PrelaunchOracle {
    pub fn update(&mut self, perp_market: &PerpMarket, slot: u64) -> VelocityResult {
