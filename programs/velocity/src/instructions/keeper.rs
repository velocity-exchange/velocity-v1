...
pub fn place_signed_msg_taker_order<'c: 'info, 'info>(
    ...
    // 500 baseline slots (~200s), inflated to actual slots
    let max_order_slot_age = effective_slots(500, state.slot_duration_ms());
...
pub fn handle_update_perp_bid_ask_twap<'c: 'info, 'info>(
    ...
    let (bids, asks) = find_bids_and_asks_from_users(
        ...
        slot,
        now,
        effective_slots(BID_ASK_TWAP_MIN_QUOTE_REST_SLOTS, state.slot_duration_ms()),
    )?;
...
pub fn handle_update_amms<'c: 'info, 'info>(
    ...
    crate::vlp::amm::refresh::update_amms(&mut perp_market_map, &mut oracle_map, &state, &clock)?;
...
pub fn handle_force_delete_user<'c: 'info, 'info>(
    ...
    {
        // threshold in 400ms baseline units (~3 months); deflate the measured
        // slot delta to match
        let slots_since_last_active = crate::math::slots::base_units_from_slots(
            slot.safe_sub(user.last_active_slot)?,
            state.slot_duration_ms(),
        );

        validate!(
            slots_since_last_active >= 18144000, // 60 * 60 * 24 * 7 * 4 * 3 * 2.5 (~3 months)
            ErrorCode::DefaultError,
