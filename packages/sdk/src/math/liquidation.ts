 ...
 * `programs/velocity/src/math/liquidation.rs`. The program sizes the asset-conversion leg of
 * spot and pnl-vs-spot liquidations with this instead of the raw oracle price whenever the
 * deposit oracle fails the margin-calc validity gate — a stale or uncertain oracle can make
 * an account liquidatable but cannot cheapen its collateral. Pass the result as `assetPrice`
 * to `calculateAssetTransferForLiabilityTransfer` to predict on-chain transfer amounts in
 * that case.
 ...
 * margin-invalid, the program prices the liability-conversion leg of spot and pnl-vs-spot
 * liquidations at this instead of the raw oracle price, so an inflated stale/uncertain debt
 * price cannot cheapen the collateral (or pnl) exchanged for it. Pass the result as
