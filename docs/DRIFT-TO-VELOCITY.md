...
- **Error codes are ABI-stable**: removed variants were renamed to `Deprecated*` stubs
  ...
  `InvalidNativePerpMarketAccount` (6356), `IsolatedPositionsDisabled` (6357),
  `HedgeDisabled` (6358). Decode errors by code as before, but expect
...
| feature-gates | Add runtime feature gates for isolated positions and the LP-pool hedge. New `FeatureBitFlags::IsolatedPositions` (bit `0b00001000`, reclaiming the bit PR #67 freed when it removed `BuilderReferral`) + `State.allow_isolated_positions()`, gating `deposit_into_isolated_perp_position` / `transfer_isolated_perp_position_deposit` / `withdraw_from_isolated_perp_position`. New `LpPoolFeatureBitFlags::Hedge` (bit `0b00001000`) + `State.allow_hedge()`, gating `update_lp_constituent_target_base`. Both default OFF; cold-admin required to enable, any hot admin may disable (kill switch); enable-guard rejects with `Unauthorized`. New instructions `update_feature_bit_flags_isolated_positions` / `update_feature_bit_flags_hedge`; new errors `IsolatedPositionsDisabled` (6357) / `HedgeDisabled` (6358). SDK: `FeatureBitFlags.ISOLATED_POSITIONS`, new `LpPoolFeatureBitFlags` enum, `AdminClient.updateFeatureBitFlagsIsolatedPositions` / `updateFeatureBitFlagsHedge`. Also completes #67's SDK cleanup: removes the dead `FeatureBitFlags.BUILDER_REFERRAL` + `AdminClient.updateFeatureBitFlagsBuilderReferral` (TS SDK) and the `StateExt::has_builder_referral_feature` helper (`velocity-rs`), which referenced the already-removed program flag. No account-layout change (flags live in existing `feature_bit_flags` / `lp_pool_feature_bit_flags` bytes) |
