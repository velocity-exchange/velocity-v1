...
/// The same property with an unrestricted, empty user set.
///
/// It does not hold today. With an empty set, `quote` cannot tell makers apart
/// by set index, so it never counts them against `max_execute_users`, while
/// `execute` stops once it has written that many balance changes. A level
/// spread over more makers than the ceiling is quoted in full and executed in
/// part. Velocity's fill path always names its users, so no fill is affected,
/// but the empty-set view overstates executable depth.
#[test]
#[ignore = "quote does not count distinct makers for an empty user set"]
            ...
            match rng.below(7) {
                ...
                1 => {
                        ...
                        orders.get(rng.below(orders.len() as u64) as usize)
                    {
                        ...
                        let bound = node.is_taker_origin() && !node.is_active(slot);
                        let result =
                ...
                3 => {
                    ...
                    if count > 0 && count >= book.evict_threshold_per_side {
                        book.evict_worst(side)
                            .unwrap_or_else(|error| panic!("{context}: evict failed: {error:?}"));
                    }
                }
                4 => {
                ...
                5 => {
                        ...
                        remainders.get(rng.below(remainders.len() as u64) as usize)
                    {
                        ...
                        book.fill(order_ref(index, &node), amount.min(node.base_asset_amount))
                            .unwrap_or_else(|error| panic!("{context}: fill failed: {error:?}"));
                    }
                }
                _ => {
