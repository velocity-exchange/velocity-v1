//! Simple example JIT(ter)
//!
//! It watches jit auctions and swift orders, and tries to fill them using the jit-proxy program.
//! The jit-proxy program will ensure fills at set prices and max/min positions (otherwise fail);
//! prices are set at a fixed margin from oracle, fills are attempted every slot until an auction is complete.
//!
//! This is provided as an example of the overall flow of JITing; a successful jit maker will require
//! tuning for optimal prices and tx inclusion.
//!
// Illustrative surface (custom-strategy constructor, Sniper stub, exclusion hook)
// is kept for reference even though this example only wires up the Shotgun path.
#![allow(dead_code)]
use std::env;
...
async fn main() {
    ...
    let velocity_client = VelocityClient::new(
        Context::MainNet,
    ...
    let jitter = Jitter::new_with_shotgun(velocity_client.clone(), Some(config), Some(cu_params));
    ...
    let fwog_perp = velocity_client.market_lookup("fwog-perp").unwrap();
