...
/// The CLOB program velocity wrote and trusts. A `Clob` quoter entry may only
/// name this program. A book settles for whoever it says rests on it, so the
/// program behind the entry is the trust root for maker identity.
/// `initialize_quoter` enforces the pin.
