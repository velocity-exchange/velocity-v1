...
///             .usermap_on()
///             .on_user_account(builder.account_update_handler(velocity.backend().account_map()))
...
impl<'a> DLOBBuilder<'a> {
    ...
    pub fn slot_update_handler(
        ...
        let notifier = self.notifier.clone();
        move |new_slot| {
            // Keep the DLOB's slot clock in sync with `State`. The clock only
            // changes when an IBRL transition is synchronized.
